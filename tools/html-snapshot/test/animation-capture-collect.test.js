...
describe('pagxCollectWAAPI', () => {
  ...
  test('samples intermediate matrices when a full turn has identical endpoints', () => {
    const { anim, target } = makeAnim();
    let currentTime = 0;
    anim.effect.getKeyframes = () => [
      { opacity: '1', transform: 'rotate(0deg)', offset: 0 },
      { opacity: '1', transform: 'rotate(360deg)', offset: 1 },
    ];
    anim.effect.getComputedTiming = () => ({
      duration: 1000, delay: 0, iterations: Infinity, direction: 'normal',
    });
    anim.pause = () => {};
