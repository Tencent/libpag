...
export function safeQueryParam(req: IncomingMessage, key: string): string | null {
  try {
...
export function wantsJsonResponse(req: IncomingMessage): boolean {
  const accept = (
...
//   - the CLI parser (lib/cli.ts's parseCookie produces well-formed entries;
...
// input.
export function validateCookies(value: unknown): CookieEntry[] {
  if (!Array.isArray(value)) return [];
  return value.filter(
    (c: unknown): c is CookieEntry =>
      !!c && typeof c === 'object'
        && typeof (c as CookieEntry).name === 'string'
        && typeof (c as CookieEntry).value === 'string',
  );
}
...
export function validateHeaders(value: unknown): HeaderEntry[] {
  if (Array.isArray(value)) {
    return value.filter(
      (h: unknown): h is HeaderEntry =>
  ...
  if (value && typeof value === 'object') {
    return Object.entries(value as Record<string, unknown>).filter(
      ([k, v]): boolean => typeof k === 'string' && typeof v === 'string',
    ) as HeaderEntry[];
  }
  return [];
}
