...
// Key for identifying unique glyphs: typeface + fontSize + glyphID.
// Different font sizes produce different glyph paths/images, so they need separate entries.
struct GlyphKey {
...
class FontEmbedderImpl {
 public:
  ...
  bool embed(const TextGlyphs& textGlyphs) {
    ...
    textGlyphs.forEach([this](Text*, std::shared_ptr<tgfx::TextBlob> textBlob) {
      if (textBlob == nullptr) {
        return;
      }
    ...
    // Create Font nodes (one for vector glyphs, one for bitmap glyphs)
    createFontNodes();

    // Second pass: create GlyphRuns for each Text with remapped glyph IDs
 ...
 private:
  ...
  void createGlyphRuns(Text* text, const std::shared_ptr<tgfx::TextBlob>& textBlob) {
    ...
    for (const auto& run : *textBlob) {
      ...
      // Separate glyphs into vector and bitmap based on their mapping
      ...
      if (!vectorIndices.empty() && vectorFont != nullptr) {
        auto glyphRun = createGlyphRunForIndices(run, vectorIndices, vectorFont, vectorGlyphMapping,
                                                 typeface, fontSize);
        if (glyphRun != nullptr) {
      ...
      if (!bitmapIndices.empty() && bitmapFont != nullptr) {
        auto glyphRun = createGlyphRunForIndices(run, bitmapIndices, bitmapFont, bitmapGlyphMapping,
                                                 typeface, fontSize);
        if (glyphRun != nullptr) {
          text->glyphRuns.push_back(glyphRun);
        }
  ...
  GlyphRun* createGlyphRunForIndices(
      const tgfx::GlyphRun& run, const std::vector<size_t>& indices, Font* font,
    ...
    // Copy positions for selected indices
