...
| `corpus/websites/` | 真实网页手写快照 | 26 个 `.html` + 必要图片 | bilibili、douyin、jd 等移动端页面的静态化版本。 |
| `corpus/generated/` | 多模型生成的演示/技术页 | 162 个 `.html` | 按模型分子目录（`case/`、`glm5.1/`、`ima-*`、`tech-*`），多使用 Tailwind CDN + web 字体。 |

### 语料源码与必要资源

本目录收录 `.html` 源文件、`cases/` 元数据，以及 HTML 在浏览器中正确渲染所必需的本地
图片和字体资源。目前整套语料约 109 MB，其中 82 个 PNG（约 97 MB）由 Git LFS 管理，
3 个小型 WOFF2 字体作为普通 Git 文件提交。克隆后需确保 Git LFS 已安装并完成资源拉取：
...
评测派生物仍不入库，包括 `*.subset.html`、评测生成的 `baseline.png` / `subset.png` /
`diff.png`、`*.pagx`、`*.pptx` 以及 `tools/html-snapshot/eval/out/`。不要删除 HTML 引用的
`images*/`、`fonts/` 等资源目录，否则浏览器基准图和 PAGX 渲染都会缺少内容。
