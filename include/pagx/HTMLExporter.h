...
enum class HTMLFormat {
  /**
   * Default behavior: HTML tags indented by `HTMLExportOptions::indent` spaces;
   * the <style> block emits one CSS rule per line without declaration indent.
   */
  Compact,

  /**
   * Developer-friendly output. HTML is indented; each CSS declaration appears
   * on its own line with a 2-space indent under its selector. Useful for
   * debugging or hand-editing generated HTML.
   */
  Pretty,

  /**
   * Production-oriented minification. All HTML newlines and leading whitespace
   * are stripped; CSS rules are concatenated on a single line; the
   * "Generated by" comment is omitted.
   */
  Minify,
};
...
struct HTMLExportOptions {
  ...
  /**
   * Indentation spaces for the output HTML. The default value is 2. Ignored
   * when `format` is HTMLFormat::Minify.
   */
  int indent = 2;
