...
class PAGXDocument : public Node {
 public:
   ...
   * and content nodes (Image, SolidColor, Gradient, ColorStop, Fill, Stroke, Group, TextBox,
   * filters, styles); a content node refreshes the Layer that contains it, so passing the owning
   * Layer is equivalent. Filters and styles match by top-level pointer only. GlyphRun, Font, and
   * Glyph nodes are not supported (they are generated by applyLayout); update Text content or font
   * properties via the Text node with layoutChanged = true.
   *
   * notifyChange only refreshes the nodes it is given, so the caller must list every node an edit
   * affects, not just the mutated one:
   * - Changing an "@id" reference (e.g. AnimationObject.target, Fill.color): include the whole
   *   reference chain.
   * - A layoutChanged edit re-runs layout for the entire document and may reposition sibling or
   *   ancestor Layers (e.g. a taller text node shifts the following items in a Vertical/Flex
   *   container); include every Layer the re-layout moves, or pass the container Layer.
   *
   * For an external composition, call notifyChange on the document that owns the edited nodes;
   * embedding scenes refresh automatically. A node may only be notified through its owning document;
   * foreign nodes are skipped (use ownsNode() when the owner is not statically known).
   *
   * @param dirtyNodes the nodes whose fields or child lists were mutated. Must be owned by this
   * document; null entries and foreign nodes are skipped; an empty list is a no-op.
   * @param layoutChanged true if any edit affects layout (size, constraints, padding, fonts, text,
   * geometry) or changes a child list — re-runs layout before refreshing. Pass false only for
   * edits that cannot move geometry (e.g. alpha, color) for a cheaper render-only refresh. Adding
   * or removing content (Rectangle, Ellipse, Path, etc.) always requires true, since new elements
   * have no resolved bounds until a layout pass. SetNodeChannel callers can derive the value from
   * RequiresLayout(NodeType, channel).
   *
   * Performance: prefer passing the owning Layer when known — resolving a content node to its Layer
   * scans all document nodes (O(N)). For batch edits, pass the Layer once.
   */
