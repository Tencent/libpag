...
static Rect map_rect_affine(const Rect& src, const float mat[16]) {
  ...
  // at the end.
  auto minMax =
      c3 + flip * min(min(c0 * src.left + c1 * src.top, c0 * src.right + c1 * src.top),
...
static Rect map_rect_perspective(const Rect& src, const float mat[16]) {
  ...
  const pk::skvx::float4 flip{1.f, 1.f, -1.f, -1.f};
  auto project = [&flip](const pk::skvx::float4& p0, const pk::skvx::float4& p1,
                         const pk::skvx::float4& p2) {
    float w0 = p0[3];
