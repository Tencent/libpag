...
| 5 VectorBaker + ElementTags | 🔄 | — | — | 三段拆分：5a Codec 端 11 element + VectorPayload（已交付，下行）；5b Path 量化 + ShapePath 完整（待）；5c VectorBaker + Mask 两趟（待）。Text/TextPath/TextModifier 整体推迟到 Phase 8 |
