...
7. **文本 runtime-shape**（Phase 16 v2.20）：`.pag` 文件只存原始 text + fontFamily/fontStyle，Inflater 端用 `TextShaper` 运行时 shape；字体由调用方通过 `FontProvider`（或全局 `pag::FontManager`）在播放端注册。与 v1 行为对齐，规避跨平台 glyph ID 不稳定性。v2.0-v2.19 原规划的"Render / OutlineAll 双模式"中 OutlineAll 推迟到 Phase 17+。
...
### 10.5 OutlineAll 模式（Phase 16 后推迟）

Phase 16 暂**不产出** OutlineAll 路径——原 Phase 8 规划的 "字体转 Path" 管线依赖 Baker 端持有 typeface + glyph outline 提取能力，属于 Phase 17+ 范围。当前 `PAGExporter::Options::FontMode::OutlineAll` 枚举值保留（ABI append-only），但 Baker 遇到时按 Render 处理 + 发 warning `UnsupportedFeature=104`；CLI 侧 `--pag-font-mode` 不暴露该选项。
...
class PAGExporter {
 public:
  struct Options {
     ...
     *   Render:      Emit text as (text, fontFamily, fontStyle) triples; Inflater reshapes
     *                at load time via TextShaper + caller-provided FontProvider. Default.
     *                (Phase 16 v2.20: formerly "emit as reference to embedded FontAsset",
     *                reworked to runtime-shape when TextBaker was rewritten — see §10.)
     *   OutlineAll:  Reserved; currently aliased to Render and emits Diagnostic
     *                UnsupportedFeature=104 warning. Will be reimplemented in Phase 17+
     *                by baking every glyph to vector paths at conversion time.
     */
...
| B3 | OutlineAll 模式：~~surfaceB 独立基准图 `PAGRenderTest_OutlineAll/{sample}`~~ | **Phase 16 v2.20 起全部 SKIP**（OutlineAll 未实现，推迟到 Phase 17+）；OutlineAll_Baseline test 保留参数化结构以便未来接入 |
...
**B3 的独立基准**（Phase 16 前）：~~OutlineAll 模式因 path A 不等价于 path B，surfaceA 不再作为 surfaceB 的参照；surfaceB 独立保存 `PAGRenderTest_OutlineAll/{sample}` 基准图。~~ Phase 16 v2.20 起 OutlineAll 未实现，本档项全部 SKIP。
...
TEST_P(PAGRenderEquivalenceTest, OutlineAll_Baseline) {
  ...
  // Phase 16 v2.20: OutlineAll path is not implemented yet; skip wholesale so
  // the parameterisation structure stays in place for Phase 17+ landing.
  GTEST_SKIP() << "OutlineAll pending Phase 17+";
  /*
  auto doc = PAGXImporter::FromFile("spec/samples/" + sample + ".pagx");
  ASSERT_NE(doc, nullptr);
  doc->applyLayout();
...
- ~~OutlineAll 模式：`PAGRenderTest_OutlineAll/{sampleName}` → 独立基准~~（Phase 16 v2.20 起 SKIP）
...
- Baseline 机制会自动把当前产出保存到 `test/out/PAGRenderTest_Render/{sample}.webp`（参考 `test/src/utils/Baseline.cpp` 的 OUT_ROOT 机制；Phase 16 v2.20 起 `PAGRenderTest_OutlineAll/` 目录不再产出）；
...
| 12 | **`.github/workflows/pagx-fuzz.yml`**（P1-6 v2.17 登记：§18.3ter CI 规划正式落地产物，4-shard matrix × 6h + corpus cache + crash auto-commit） | `RenderEquivalenceTest` + `PAGDecodeFuzzTest`（§18.3ter Layer 6 Fuzz，≥ 1 CPU·小时 ASAN/UBSAN 全绿） | Render 模式 Baseline::Compare 全绿；Fuzz 全绿；CI yaml 首次上绿（OutlineAll 分支 Phase 16 起 SKIP） |
...
**Phase 12 首次提交基线缺失说明**（P1-10 v2.18）：`RenderEquivalenceTest` 首次跑时 `test/baseline/PAGRenderTest_Render/` 目录**根本不存在**——48 个样本全部首跑 FAIL 为预期。交付判定应分两步：(a) tech lead 人工审图通过（可用 `tools/render_compare.py` 生成两列对比 HTML）；(b) 用户执行 `/accept-baseline` 批量接受基线，再回跑转绿。AI 遇首跑 FAIL 不应误判为 bug，而应按此流程处理。**Phase 16 v2.20 起 `PAGRenderTest_OutlineAll/` 分支全部 SKIP**（OutlineAll 未实现），不再产出这一基准目录。
...
3. **OutlineAll 模式比对策略**：~~定案——**不走 memcmp**；Render 和 OutlineAll 各自走 `Baseline::Compare` + 独立基准图 folder（见 §17 B 档 + §18.7）。~~ **Phase 16 v2.20 起 OutlineAll 路径未实现**（推迟到 Phase 17+）；只保留 Render 分支。
...
enum class DiagnosticCode : uint16_t {
  ...
  TextGlyphDataEmpty        = 206,  // pagx::Text 的 glyphData 为空但仍有 text 字段；Phase 16 起已可走 runtime shape，本码仅保留
                                    //   给 Phase 8 残留代码路径，Baker 正常流不再触发
...
历史完整修订记录（v1.0 → v2.20）见 [`docs/pagx_to_pag_v2_changelog.md`](pagx_to_pag_v2_changelog.md)。**实现阶段不需通读历史**，读完上述 16 条（12 基线 + 3 v2.19 新增 + 1 v2.20 新增）即可开工。
