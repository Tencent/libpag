...
[编码]
  *.pagx (XML)
  ...
  PAGDocument
    │
    ▼ pagx::pag::Codec::Encode       [EncodeContext]    ◀── 新增
  *.pag (version = 0x02)
...
### 3.2 四态三映射四 Context 全览（接手 AI 5 分钟必读）
...
> **不预留无 roadmap 段**：240-299 段（原"资源扩展"预留）v2.20 起删除——未规划的 SVG / Lottie / HDR 等资源类型不应占据文档化席位；真正需要时按 §6.5 字段级追加规则新划段即可。分段历史变更（Filter 段 v1 70-74 → v2 80-99 → 当前 120-139 等）见 CHANGELOG。
...
>
> Decoder 遇到老版本开发分支产出的 `FontAssetTable` 走 `UnknownTagCode=400` warn + length skip，不构成阻塞（见 §6.2 / §10）。
...
- **4 Context 形态对称**：都继承 DiagnosticCollector、都有 3 参 public `warn`。调用点永远 `ctx->warn(...)`，无"指针聚合体 vs Context"的风格分叉。所有 `Write<TagName>` 签名第三参为 `EncodeContext* ctx`。
...
// ---------- EncodeContext（Encode 阶段，无 fatal） ----------
...
| `InflateFontCreateFailed` (601) | `inflateElementText` 内 `FontProvider::getTypeface(fontFamily, fontStyle)` 返回 null 且 fallback 链也无可用 typeface | 该 ElementText 渲染为空白占位 |
| `InflateGlyphRunBuildFailed` (602) | `inflateElementText` 内 `TextShaper::Shape` 返回空 glyph 列表或段落布局产生 null TextBlob | 该 ElementText 被跳过，其外层 VectorGroup 其他子元素仍渲染 |
...
| ~~FontAsset~~ | Phase 16 v2.20 废弃（runtime-shape，字体由 `FontProvider` 注入；见 §10） | — | — | — | — |

**去重原则**：同内容多次引用只入表一次；VectorElement 通过 `uint32_t imageAssetIndex` 引用。详细字段定义见 §11.1；索引化流程见 §11.2。字体在 Phase 16 runtime-shape 模式下由 `ElementTextData::fontFamily / fontStyle` 字符串直接携带，不再有独立资源池（§10）。
...
ResourceBaker pre-pass 查表顺序：先 `imageIndexByNode[nodePtr]`，不命中依据 PAGX Image 来源分支——内嵌走 `imageIndexByDataPtr`，URI/文件走 `imageIndexByKey`——任一命中即复用 index，全 miss 才 `push_back` 新 ImageAsset。规模估算：图标模板被 50 Layer 共享的场景，从 50× push 降为 1× push + 49× Layer 2 命中，省 ~245 MB（50 × 5 MB PNG）+ 49 次解码。
...
- **Options 独立（不合并 CommonOptions）**：`PAGExporter::Options` / `PAGLoader::Options` 保持独立定义——虽然都含 `strict` 字段，但承载字段集随角色分化（例如 Loader 需 `fontProvider` / `maxLayerBudget` 等 Exporter 无关字段），强行合并为 `pagx::CommonOptions` 会反复撕裂语义。**不合并**是刻意设计，后续评审勿重复提议。
...
**CapturePagxLayout 实现方案**（PAGExporter 测试钩子，debug build only）：
...
| `GlyphRunBlobCodecTest.cpp` | （整文件标 DISABLED） | GlyphRunBlob 结构随 Phase 16 v2.20 废弃；Phase 16.3 合入后整个测试文件删除 |
...
**CR 维护增量清单**——任何 PR 涉及 Diagnostic 改动或新 Phase 交付物：
1. 新增 `DiagnosticCode` 走 §19 Phase 0 行的五步清单 (a)-(e)（枚举、kAllDiagnosticCodes、CodeToString、§G.6 矩阵、§15.1 ABI 表）；
2. 修订日志加条目到 `docs/pagx_to_pag_v2_changelog.md`（版本号 / P 级别 / 修订说明）；
3. 新术语补 §3.3；
4. 若影响 Phase 工件顺序，同步 §19 Phase 依赖矩阵。
...
**Phase 4 拆分**（可选实施）：若 Phase 4 一次吞"FileHeader + ImageAssetTable + Composition + LayerBlock"过大，可拆为：
- **Phase 4a**：容器头 + FileHeader + End + CompositionList（仅框架 + 空 composition 可走通）
- **Phase 4b**：ImageAssetTable + Composition + LayerBlock 主体
...
早期评审的 7 条"待确认"已全部定板，当前生效规则直接去权威章节查：

| 主题 | 权威章节 |
|---|---|
| 文本序列化字段集（runtime-shape） | §10 / §C.7 / §D.11 |
| ColorMatrix 20-float 特化 | §D 的 `WriteValue<std::array<float, 20>>` |
| pag v1 播放器对 0x02 的 graceful reject | `src/codec/Codec.cpp::ReadBodyBytes:196` `KnownVersion` 检查（无需改动） |
| 样本覆盖面 | §18.9（用户补样本时附特性矩阵） |
| tgfx::Path 序列化 | §D `WriteValue<tgfx::Path>` / §D.2 |
| bit-exact 渲染验证 | §18.2 Day-1 smoke |
| OutlineAll 路径 | v2.20 起已从 API / CLI / 测试移除（见 §15.2） |
...
- §D.11 `ElementText.text` / `fontFamily` / `fontStyle`（`text` 走 `MAX_TEXT_STRING_BYTES`；`fontFamily` / `fontStyle` 走 `MAX_NAME_STRING_BYTES`）
...
**ElementText body 布局**：ElementText body 直接携带 v1 TextDocument 风格字段（见上表 "ElementText (50)" 行），不涉及动态位宽量化——所有字段均为 ValueCodec 标准原语（`Property<Point>` / `utf8string` / `Color` 等），直写即可。TextShaper runtime shape 的典型开销：200 字符段落 < 1 ms，相对 tgfx GPU 渲染总开销可忽略。
...
enum class DiagnosticCode : uint16_t {
  ...
  FontSourceMissing         = 201,  // pagx::Text 的 fontFamily 为空字符串，Baker 无法记录字体名（Inflater 将依赖 FontProvider fallback）
  ...
  TextGlyphDataEmpty        = 206,  // 废弃（v2.20 起不再产出）；枚举值保留仅为 ABI append-only 约束，新代码禁引用
  EmptyDocument             = 207,  // 空 pagx（non-fatal informational）
  TextGlyphRunsDowngraded   = 208,  // pagx::Text.glyphRuns 非空（pre-shaped），Baker 丢弃 glyph ID + per-glyph xform，
                                    //   降级为 text + fontFamily 的 runtime-shape 路径
  ...
  UnknownTagCode            = 400,
  UnknownPropertyEncoding   = 401,
  ImageResourceSizeExceeded = 402,  // 图片单张字节数超 MAX_IMAGE_BYTES
  StringInvalidUtf8         = 403,
  PathVerbLimitExceeded     = 404,
  GlyphCountLimitExceeded   = 405,
  LayerDepthLimitExceeded   = 406,
  InvalidEnumValue          = 407,  // u8 枚举值域外数值，已回填默认值
  FontResourceSizeExceeded  = 408,  // 字体单份字节数超 MAX_FONT_BYTES（保留码值，Phase 16 runtime-shape 模式下不再触发）
  PrematureEndTag           = 409,  // 中途 End Tag 截断（见 §8.3）

  // ---------------- Inflater 告警 (600-699) ----------------
  InflateImageDecodeFailed  = 600,  // tgfx::Image::MakeFromEncoded 返回 null
  InflateFontCreateFailed   = 601,  // FontProvider.getTypeface 返回 null 且 fallback 链也无可用 typeface；Text element 渲染为空白
  InflateGlyphRunBuildFailed = 602, // TextShaper::Shape 返回空 / 生成的 TextBlob 为 null；整个 ElementText 跳过
  ...
  InflateMaskCycle          = 607,   // Layer mask 循环；环上 mask 全部跳过
};
}  // namespace pagx
