...
| **3 Context**（v2.18 收敛；原 4 Context，Encode 退化为 DiagnosticCollector + StreamContext 组合）| `BakeContext`（Baker 用） / `DecodeContext`（Decode 用） / `InflaterContext`（Inflate 用）——均继承 `DiagnosticCollector`；Encode 阶段直接用 DiagnosticCollector + StreamContext 组合，不再独立 Context——见 §8.5 / §9.4 |
...
- 3 Context 全部继承 `DiagnosticCollector` 基类（P0-3 v2.18 抽取）——统一 `MAX_DIAGNOSTICS` cap、`MAX_DIAGNOSTIC_MESSAGE_BYTES` 截断、push_back 构造 4 字段。DecodeContext 另持 `currentStream` 并补便利 wrapper 自动填 byteOffset；BakeContext / InflaterContext 的 byteOffset 恒 0。
...
| `InflaterContext` | Inflater 的 Context 对象，持 warnings + layerByPath + pendingMasks | §9.4 |
...
| bit 6-7 | reserved    | **P2-8 v2.18 语义预定**：Writer **必须**写 0；旧 Reader **必须**忽略。未来扩展通道：若 `hasExt` / encoding 池不足，可把 bit 6 定义为 "propHeader continuation"——bit 6=1 时再读 1 byte `propHeader2` 承载 encoding 扩展段 + 色彩空间等；bit 7 仍预留。这条通道**独立于** `hasExt` 红线（§4.3 hasExt 永久冻结 1 byte），避开单方面扩展 extHeader 尺寸的结构性破坏。本期不激活，仅保留语义设计空间 |
...
**规则 1**（`encoding` 位段非 0，本期未知动画编码）：**按粒度分层 skip**（P0-5 v2.18）——
  - **Layer 级 Property**（`alpha` / `visible` / `blendMode` / `matrix` / `matrix3D`）：Writer 端必须把这些 Property 打包进**专属子 Tag** `TagCode::LayerTransform = 15`（§6.1 Layer 段新增）；本期 Decoder 见未知 encoding 时 `stream->setPosition(layerTransformTagEnd)`，**仅丢 transform，不丢整 LayerBlock**——Layer children / payload 仍能读出。
...
| Layer 及子 Tag | LayerBlock / LayerMaskRef / LayerFilters / LayerStyles / LayerTransform / 保留 | 10-19 | 5 (10,12,13,14,15) | 5 个 |
...
**第三方实验段注册纪律**（P1-5 v2.18）：900-1021 区段采用**命名空间隔离**——任何实验 Tag body 必须以 **16 byte namespace UUID**（RFC 4122 v4）作为 body 前 16 字节前缀，Decoder 对 UUID 不认识即整 Tag 走 `UnknownTagCode=400` warn + length skip。同一 TagCode 数值 + 不同 UUID 视为不同扩展，**消除 fork 生态冲突**。官方 Tencent 扩展使用 UUID `namespace.pagx.tencent.com`；第三方必须生成独立 UUID 并向上游注册以避免重复。Decoder 读入布局：
...
**必选 top-level Tag 排除清单**（P0-4 v2.18，强制）：**FileHeader / ImageAssetTable / FontAssetTable / CompositionList 四个 top-level Tag 必选**——旧 Reader 对这四个中任何一个走 UnknownTagCode skip = 丢失 canvas 尺寸 / 资源池 / composition 列表 = **整文件不可播**（非视觉降级）。因此：
- 这四个 Tag 的**字段语义不兼容变更**禁止走 ②，必须升 FORMAT_VERSION（③）；
...
class Codec {
 public:
  // Encode 内部自建 EncodeContext（Encode 失败只产生 warnings，不产生 errors；无需调用侧传策略位）。
...
**v2.18 简化说明**（P0-2 + P0-3，强制）：
- **EncodeContext 合并**（P0-2）：原 EncodeContext 仅持 warnings + streamContext + depth 字段，depth 检查重复 Baker 职责——v2.18 起 Encode 阶段直接用 `DiagnosticCollector + StreamContext` 组合，不再独立定义 EncodeContext。对外 API（PAGExporter::Result.warnings）签名不变，仅内部收敛。§3.2 "4 Context" 叙述相应更新为 "3 Context"（BakeContext / DecodeContext / InflaterContext）。
- **DiagnosticCollector 基类抽取**（P0-3）：下述四处的 warn / error 方法字字相同（MAX_DIAGNOSTICS cap + MAX_DIAGNOSTIC_MESSAGE_BYTES 截断 + push_back 构造 4 字段）——提炼到 `struct DiagnosticCollector { vector<Diagnostic> errors, warnings; void error(...); void warn(...); bool hasError(); }`，BakeContext / DecodeContext / InflaterContext 以 `: DiagnosticCollector` 继承；DecodeContext 多出 `currentOffset()` 便利 wrapper 自动填充 byteOffset。实现时优先落 DiagnosticCollector，再按下述定义拼接各自专属字段（depth / 全局计数 / pendingMasks 等）。

下方 EncodeContext / DecodeContext / BakeContext 的字段清单与方法规约仍以 v2.17 版本为参考——落实现时按上述两项收敛；InflaterContext 见 §9.4（同批继承 DiagnosticCollector）。
...
struct EncodeContext {
  ...
  void warn(ErrorCode code, std::string msg = {}, uint32_t contextIndex = UINT32_MAX) {
    ...
    warnings.push_back({code, std::move(msg), 0, contextIndex});  // Encoder 不追踪 byteOffset
  }
};
...
struct DecodeContext {
  ...
  void warn(ErrorCode code, std::string msg = {}, uint32_t contextIndex = UINT32_MAX) {
    ...
    warnings.push_back({code, std::move(msg), currentOffset(), contextIndex});
  }
...
struct BakeContext {
  ...
  void warn(ErrorCode code, std::string msg = {}, uint32_t contextIndex = UINT32_MAX) {
    ...
    warnings.push_back({code, std::move(msg), 0, contextIndex});
  }
  ...
  bool registerComposition() {
    if (totalCompositionCount >= limits::BAKE_MAX_TOTAL_COMPOSITIONS) {
      error(ErrorCode::StructureLimitExceeded, "BAKE_MAX_TOTAL_COMPOSITIONS exceeded");
      return false;
    }
...
struct ImageAsset {
  ...
  int width = 0;
  int height = 0;
  // P1-2 v2.18：字段级追加（§6.5 ①），走 Raster 兼容语义；本期 Baker 恒写 Raster=0
...
struct FontAsset {
  ...
  std::shared_ptr<const tgfx::Data> data;
  // P1-2 v2.18：字段级追加（§6.5 ①）——Variable Font 轴。本期 Baker 恒写空数组；旧 Reader 按 TagHeader.length skip。
 ...
 *   - shared_ptr<const tgfx::Data>：比较 size + 字节序列；
...
// Mask chain helper（P0-10 v2.18）：构造单个 Layer 挂 k 个 mask path 引用（测 MAX_PENDING_MASKS 熔断）。
// PAGXBuilder.withMask 原本单次调用只设一条 maskLayerPath；本 helper 模拟"多个 maskLayerPath 候选"——
// 实际产出为 k 个独立兄弟 Layer，每个 mask 指向第 0 层。验证 reservePendingMaskSlot 在 k > 262144 时生效。
inline std::unique_ptr<pagx::PAGXDocument> MakeLayerWithKMasks(PAGXBuilder&& builder, int k) {
  builder.openComposition("root");
  builder.enterLayer(pagx::LayerType::VectorLayer);
  ...
  for (int i = 0; i < k; ++i) {
    builder.enterLayer(pagx::LayerType::VectorLayer);
    builder.withName(std::string("M") + std::to_string(i));
    builder.withMask(pagx::LayerMaskType::Alpha, "root/0");   // 全部 mask 指向第 0 层
    builder.exitLayer();
  }
  builder.closeComposition();
  return builder.Build();
...
- `ZeroCopyScope.LeakingDataToTreeIsBlocked`：故意让 `MakeWithoutCopy` 产物逃逸到 `PAGDocument::images[0]->data`——Inflater 调 `tgfx::Image::MakeFromEncoded` 后 PAGLoader 释放 backing buffer，ASAN 应该 catch 到 use-after-free（本条是 **ASAN-positive regression 测试**，确认 ZeroCopyScope 硬约束生效；若 fix 完整 ASAN 绿 = 测试失败？实际写法：用 `testing::GTEST_FLAG(death_test_style)` 或反向断言 "此测试预期 fail" 配合 `EXPECT_NONFATAL_FAILURE` 等工具）。
...
uint8_t ChoosePathFormat(const tgfx::Path& p) {
  ...
  if (verbCount < limits::PATH_QUANTIZATION_MIN_VERBS) return 0;   // 极短直接 format=0
...
// P1-1 v2.16 强制：坐标/权重合法性校验辅助——任何 NaN/Inf/超 int24 立即 warn MalformedTag + 返回空 Path
auto checkCoord = [](float v) -> bool {
  return std::isfinite(v) &&
         std::abs(v) < static_cast<float>(limits::PATH_QUANTIZATION_MAX_COORD);
};
#define READ_POINT_SAFE(ptr)                                           \
  do {                                                                 \
    ptr = ReadPoint(stream);                                           \
    if (!checkCoord(ptr.x) || !checkCoord(ptr.y)) {                    \
...
**理由**：format=0 的坐标和 conicWeight 读 `readFloat()` 直出，未做任何校验；tgfx::Path 接受 NaN/Inf 后续 rasterizer/PathMeasure 进入 UB（部分实现 `while(!converged)` 死循环），`int32_t(float)` 转换在越界 float 上也是 UB。format=1 量化格式天然受 int24（`PATH_QUANTIZATION_MAX_COORD`）钳制，无需补。
...
inline uint8_t ChoosePathFormat(const tgfx::Path& path) {
  ...
  int64_t limit = limits::PATH_QUANTIZATION_MAX_COORD;
  bool overflow = false;
...
body:
  ...
  repeat assetCount:
    ...
    // P1-2 v2.18 末尾追加：kind (u8)，非 0 = SVG/Video/Hdr 扩展；旧 Reader 按 length skip
    u8 kind         # 0 = Raster (default); reserved: 1 Svg / 2 Video / 3 Hdr
```
...
body:
  ...
  repeat assetCount:
    ...
    utf8string  family
    utf8string  style
    varU32      dataLen         # kind == Embedded 时非零；kind == Embedded 时 Decoder 必须走 §H.5 magic 白名单（P0-17 v2.18）
    dataLen bytes
    # P1-2 v2.18 末尾追加：Variable Font 轴数组（本期 Baker 恒写 axisCount=0；旧 Reader 按 length skip）
    varU32      axisCount       # 轴数量（本期恒 0）
    repeat axisCount:
...
- v2.17 → v2.18 修订要点（5 位专家评审团第 9 轮综合：17 P0 + 16 P1 + 10 P2 = 44 项，**用户要求全部 33 项清零 + 11 项新边界追加**）：
  ...
  - **CorruptBuilder 扩 4 方法**（P0-11）：`AppendErrorMarkerTag / SetCompressionByte / SetBodyLength / SetCompositionIndexAt / AppendGarbageAfterEnd` 消除 41 码中 106/302/305/306/409 触发路径缺口。
  - **可扩展性预补**（2 年后演进）：
