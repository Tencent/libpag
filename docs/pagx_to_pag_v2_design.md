...
**6 个 Inflater 告警码的触发点**（与附录 G.2 的 600-699 段严格一一对应）：
...
| `InflateCompositionCycle` (605) | `CompositionVisitScope` 检测到自环 / 超 `MAX_COMPOSITION_REF_DEPTH=32` | 该 CompositionRef 降级为空 Layer，其余兄弟节点继续渲染 |
...
class PAGLoader {
 public:
  ...
  struct PeekResult {
    ...
    int      canvasWidth  = 0;
    int      canvasHeight = 0;
    uint16_t formatVersion = 0;       // 0x0002 for PAG v2 (low byte = major, high byte = minor)
    std::vector<Diagnostic> errors;
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
  ...
  if (r.doc) {
    (void)pagx::pag::LayerInflater::Inflate(*r.doc);
  }
...
| 4 | `Codec.cpp` 基础 Tag（FileHeader/Composition/LayerBlock） | `RoundTripTest` 前半 + `VersionRejectTest` + `TruncatedDecodeTest`（**仅** 300/301/302/303/305/306/307 段，Path/Glyph/Resource 相关 402/403/404/405 推迟到对应 Phase） | 全绿 |
...
| 9 | `LayerInflater.cpp` + `support/RenderUtil.cpp` | `InflaterParityTest` + `PAGDocumentParityTest` + `InflaterParity.CompositionSelfRef / CompositionRefTooDeep`（605） | 全绿 |
