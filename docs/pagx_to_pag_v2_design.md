...
9. LayerInflater 设计（LayerDoc → tgfx::Layer）
10. 字体双模式与文本处理
11. 资源管理（图片 / 字体）
12. Mask 两趟处理
13. 动画可扩展性保留
...
21. 附录 B：开工前待确认事项
22. 附录 C：LayerDoc 完整 C++ 定义
23. 附录 D：Tag 字节布局规范
24. 附录 E：枚举映射表（PAGX ↔ LayerDoc ↔ tgfx）
25. 附录 F：字段名对照表（LayerBuilder ↔ PAGX DOM ↔ LayerDoc ↔ tgfx）
26. 附录 G：错误码与诊断
27. 附录 H：安全上限与资源约束
...
- 四态：`PAGXDocument` → `LayerDoc` → `bytes` → `LayerDoc` → `tgfx::Layer`
- 三映射：`Baker`（PAGXDocument→LayerDoc） / `Codec`（LayerDoc↔bytes） / `LayerInflater`（LayerDoc→tgfx::Layer）
...
> **权威定义**：所有 LayerDoc 结构体、枚举、默认值的 C++ 完整定义在**附录 C**。本章只做概念描述，不再出现代码片段——AI 编码期间**按附录 C 落代码**，不看本章的伪代码推断。
...
|---|---|---|---|
| 顶层入口 | `Baker.cpp` | 构造 LayerDoc / FileHeader / 编排两趟 | 137-163 |
...
- 所有对 LayerDoc 字段的赋值**按 LayerBuilder 的 setter 顺序排布**，便于和 LayerInflater 同步修改。
...
3. **Main pass**：递归 Baker 子模块产出 LayerDoc 子树。
...
struct Diagnostic {
  ...
  std::string message;           // 人类可读补充描述
  uint32_t byteOffset = 0;       // Decoder 路径：stream->position() 快照；Baker 路径：0
};
...
struct DecodeResult {
  std::unique_ptr<LayerDoc> doc;             // 成功时非空；errors 非空时必为 nullptr
...
class Codec {
 public:
  static EncodeResult Encode(const LayerDoc& doc);
...
3. 构造 DecodeStream 覆盖 body 区间，循环读 Tag：
   ...
   ```
4. 装配完成 LayerDoc，返回 DecodeResult。
...
**规范**：**所有 Bake/Decode 入口必须用 `std::unique_ptr<LayerDoc>` 管理根对象**，fatal 时通过 `.reset()` 显式释放，不依赖函数退栈自动析构（因为 `doc` 会在成功路径被 move 出去）。
...
BakeResult Baker::Bake(const pagx::PAGXDocument& pagxDoc, BakeContext* ctx) {
  BakeResult result;
  auto doc = std::make_unique<LayerDoc>();       // ✅ 统一用 unique_ptr
  ...
  if (ctx->hasFatal()) {
    doc.reset();   // 已 push 的 compositions[]/layers[] 随 LayerDoc 析构链式释放
    result.warnings = std::move(ctx->warnings);
    result.errors   = std::move(ctx->errors);
    return result;
  }
...
**Decode 路径（Codec）**：镜像相同模式——用 `std::make_unique<LayerDoc>()` 构造根，主循环任一 Tag 产生 error 立即 `doc.reset() + return`。

**关键约束**：
1. **`BakeResult` / `DecodeResult::doc` 字段类型为 `std::unique_ptr<LayerDoc>`**（非裸指针，非 shared_ptr）；
...
5. **LayerDoc 析构链**：`unique_ptr<LayerDoc>` 析构时，其 `std::vector<std::unique_ptr<Composition>>` 自动析构，Composition 的 `std::vector<std::unique_ptr<Layer>>` 递归析构，链式释放所有子对象 —— **无需手写任何 delete**。

**禁止反面模式**：
```cpp
// ❌ 禁止：栈对象会在返回前被 move，但 fatal 路径可能不 move
LayerDoc doc;  ResourceBaker::bake(pagxDoc, doc, ctx);
if (ctx->hasFatal()) return {nullptr, ...};   // doc 在此被析构 ✓ 但让代码 intent 模糊

// ❌ 禁止：裸 new 没有 RAII 保障
LayerDoc* doc = new LayerDoc();
if (fatal) { delete doc; return {}; }         // 遇异常/早退路径易漏 delete

// ❌ 禁止：shared_ptr 引入不必要的原子计数开销（LayerDoc 是独占所有权）
auto doc = std::make_shared<LayerDoc>();
```
...
- 未知 PropertyEncoding → 按 §4.4 兜底策略：立即 seek 到**包裹该 Property 的最外层 Tag 末尾**，此 Tag 其余字段保持 LayerDoc 默认值（**不继续读取**），warn `UnknownPropertyEncoding`；
...
class LayerInflater {
 public:
  static std::shared_ptr<tgfx::Layer> Inflate(const LayerDoc& doc);
};

}
```

**无 frame 参数**（本期静态）。动画期扩展为 `Inflate(doc, frame)`，内部走 `PropertyEvaluator::Evaluate(prop, frame)` 分支。

### 9.2 实现策略

和 `LayerBuilder::build`（行 137-163）严格同构的两趟：

```
Pass 1: 按 compositions[0] 构造 tgfx::Layer 树
        - 每个 LayerDoc::Layer：
            * 按 type 派发到 tgfx::Layer::Make / VectorLayer::Make / ...
            * applyCommon(layerDoc, tgfxLayer) 逐字段 setter（镜像 LayerBuilder::applyLayerAttributes 731-796）
...
GlyphRunBlob {
  ...
  uint32 fontIndex         // 指向 LayerDoc.fonts
  float  fontSize
  bool   vertical
  varint count
  repeat count:
...
- **不产出** `ElementText`；涉及的 Font 节点不进入 `LayerDoc::fonts`（ResourceBaker 只收集被 Render 路径引用的字体）。
...
**来源三分支**（对齐 LayerBuilder::convertImagePattern 543-571）：
...
3. 打印 warnings 到 stderr；
4. `--pag-strict` 下 warning 视为 error，退出码非 0。

---

## 15. 对外 API

**唯一公共头**：`include/pagx/PAGExporter.h`
...
class PAGExporter {
 public:
  ...
  /**
   * Export result with diagnostics. On success, `bytes` is populated (for ToBytes)
   * or the file is written (for ToFile) and `errors` is empty. `warnings` may be
   * non-empty in both success and failure cases.
   *
   * All fields are owned by this struct; safe to move and discard.
   */
  struct Result {
    bool ok = false;
    std::vector<uint8_t> bytes;            // Only populated by ToBytes(); empty for ToFile()
    std::vector<std::string> errors;       // Non-empty when !ok
    std::vector<std::string> warnings;     // May be non-empty regardless of ok
  };
...
**依赖约束**（强制）：`include/pagx/PAGExporter.h` **不得 include 任何 tgfx 头文件**，保持 SDK 用户不被迫依赖 tgfx。内部实现头（`src/pagx/pag/` 下）可自由 include tgfx。

**不对外暴露**：`LayerDoc`、`Codec`、`LayerInflater`、`Baker`——全部作为内部实现细节放在 `src/pagx/pag/`。
...
src/pagx/pag/
├── LayerDoc.h                     # LayerDoc / Layer / VectorElement / Property 等全量定义
...
├── LayerInflater.h / .cpp         # LayerDoc → tgfx::Layer
└── InflaterContext.h              # pendingMasks / layerByPath 等
...
- 新增静态库 `pagx-pag`（`src/pagx/pag/` 全部 + `include/pagx/PAGExporter.h`）；
- `pagx-cli` 链接 `pagx-pag`；
- `PAGFullTest` 链接 `pagx-pag` + 新增 test 源文件。
...
| A5 | `LayerInflater::Inflate(...)` 产出非空 tgfx::Layer 树 | 结构与 LayerBuilder 树同构 |
...
| ID | 场景 | 断言 |
|---|---|---|
| C1 | 空 pagx | 全链路不 crash，产出空 tgfx::Layer 树 |
...
| Integration | §18.5 | 同上 + `LayerDocEquals` helper | LayerDoc 对象往返相等 | `EXPECT_TRUE(LayerDocEquals(a, b))` |
...
> **目的**：v2.0 引入 7 个压缩/扩展机制（A1/A3/B1/B2/C1/C2/D2），错误码路径已由附录 G.6 矩阵覆盖，但**成功路径的往返正确性 + 边界行为**需要独立测试。**这些测试放在 `test/src/pag/codec/` 目录下**，与 Baker 单测并列，不依赖 PAGX 样本，纯 LayerDoc 构造驱动。
...
- 不依赖 spec/samples；构造最小 LayerDoc / 最小 tgfx::Path / 最小 ShapeStyleData 驱动；
...
| 测试层 | 使用工具 | 断言什么 |
|---|---|---|
| **Layer 1（§18.4、§18.4bis）** | `EXPECT_EQ` / `EXPECT_NEAR` / `EXPECT_TRUE` | 字段值、字节长度、字节内容、LayerDoc 对象相等 |
| **Layer 2（§18.5）** | 同上 + 手写字节流对比辅助 | LayerDoc 往返对象语义相等 |
...
TEST(RoundTrip, VectorLayerSimple) {
  pagx::pag::LayerDoc doc1 = BuildMinimalVectorLayerDoc();  // helper
  ...
  EXPECT_TRUE(LayerDocEquals(doc1, *decoded.doc));  // deep field-by-field
}
...
TEST_P(PAGRenderEquivalenceTest, Render_Baseline) {
  ...
  ASSERT_NE(decoded.doc, nullptr);
  auto rootB = pagx::pag::LayerInflater::Inflate(*decoded.doc);
  auto surfaceB = RenderLayerToSurface(context, rootB, doc->width, doc->height);
...
| ❌ | ❌ | **Path B 存在真实 bug**（Baker/Codec/Inflater 某处有映射错误）；优先看 Path B 的 Baker 日志、再看 LayerDoc dump、最后看字节流 |
...
| 2 | `LayerDoc.h` / `BakeContext.h+cpp` / `ResourceBaker.cpp` | `BakeContextTest` / `ResourceBakerTest` | 全绿 |
...
| 9 | `LayerInflater.cpp` + `support/RenderUtil.cpp` | `InflaterParityTest` + `LayerDocParityTest` | 全绿 |
| 10 | `PAGExporter.cpp`（对外 API） | — | API review 过 |
...
|---|---|---|---|
| 顶层 build | 137-163 | `Baker::Bake` | `LayerInflater::Inflate` |
| Layer 分发 | 166-202 | `LayerBaker::bake` | `inflateLayer` |
| Composition | 204-216 | `LayerBaker`（同文件） | `inflateComposition` |
| VectorLayer | 219-231 | `VectorBaker::bakeVectorLayer` | `inflateVectorLayer` |
| Element 派发 | 262-307 | `VectorBaker::bakeVectorElement` | `inflateVectorElement` |
| Rectangle | 309-317 | `bakeRectangle` | `inflateRectangle` |
| Ellipse | 319-326 | `bakeEllipse` | `inflateEllipse` |
| Polystar | 328-344 | `bakePolystar` | `inflatePolystar` |
| Path | 346-368 | `bakePath` | `inflatePath` |
| Text（独立） | 370-385, 236-243 | `TextBaker::bakeText` | `inflateText` |
| Fill | 387-410 | `bakeFill` | `inflateFill` |
| Stroke | 412-446 | `bakeStroke` | `inflateStroke` |
| ColorSource 派发 | 448-481 | `PaintBaker::bakeColorSource` | `inflateColorSource` |
| LinearGradient | 506-514 | `bakeLinearGradient` | `inflateLinearGradient` |
| RadialGradient | 516-523 | `bakeRadialGradient` | `inflateRadialGradient` |
| ConicGradient | 525-532 | `bakeConicGradient` | `inflateConicGradient` |
| DiamondGradient | 534-541 | `bakeDiamondGradient` | `inflateDiamondGradient` |
| ImagePattern | 543-571 | `bakeImagePattern` | `inflateImagePattern` |
| Gradient stops 兜底 | 483-496 | `ExtractGradientStops` 复刻 | 同 |
| TrimPath | 573-582 | `bakeTrimPath` | `inflateTrimPath` |
| TextPath | 584-602 | `TextBaker::bakeTextPath` | `inflateTextPath` |
| RoundCorner | 604-608 | `bakeRoundCorner` | `inflateRoundCorner` |
| MergePath | 610-616 | `bakeMergePath` | `inflateMergePath` |
| Repeater | 618-630 | `bakeRepeater` | `inflateRepeater` |
| TextModifier | 632-679 | `VectorBaker::bakeTextModifier` | `inflateTextModifier` |
| RangeSelector | 661-673 | 嵌 `bakeTextModifier` | 嵌 `inflateTextModifier` |
| Group | 681-729 | `bakeGroup` | `inflateGroup` |
| TextBox | 248-260, 296-303 | `TextBaker::bakeTextBox` | `inflateTextBoxAsGroup` |
| applyLayerAttributes | 731-796 | `LayerBaker::bakeCommon` | `applyLayerAttributes` |
| LayerStyle 派发 | 798-835 | `StyleFilterBaker::bakeStyle` | `inflateStyle` |
| LayerFilter 派发 | 837-870 | `StyleFilterBaker::bakeFilter` | `inflateFilter` |
...
**Color 序列化说明**：LayerDoc **内存层**使用 `tgfx::Color`（4 × f32 RGBA，`tgfx/core/Color.h:34-124` 的 `RGBA4f<Unpremultiplied>`），但**字节流层**使用 **4 × u8 量化 RGBA**（相比 v1 `WriteColor` 的 3 × u8 RGB 增加 alpha 通道；相比 f32×4 每色省 12 B）。理由：
...
**核心原则**：LayerDoc 存储 **tgfx 枚举语义**（使用 C.1 的 `using`）；PAGX → tgfx 的映射在 Baker 一次完成，Inflater 直读。
...
**Baker 侧强约束**：所有 `PAGX::BlendMode` 字段**必须**经过 `ToTGFX(mode)` 转换后再 `MakeProp` 存入 LayerDoc；Inflater 直读 tgfx 枚举，无需二次转换。
...
PAGX `TextAlign::{Start, Center, End, Justify}`（`include/pagx/types/TextAlign.h:27-45`）→ LayerDoc `pagx::pag::TextAlign` 同名（C.1 已重定义以避免头文件污染）。tgfx 的 TextLayer 渲染处自行解析。
...
LayerDoc 直接 `using RepeaterOrder = tgfx::RepeaterOrder;`，Baker 调 `ToTGFX(order)` 一次性转换。
...
### F.1 Layer 通用字段（对应 LayerBuilder.cpp:731-796）

| LayerBuilder | PAGX `Layer` 字段 | LayerDoc `Layer` 字段 | tgfx `Layer` setter | 语义规则 |
|---|---|---|---|---|
...
| 764-766 scrollRect | `scrollRect, hasScrollRect` | `scrollRect (Property<Rect>) + hasScrollRect (bool)` | `setScrollRect` | Inflater 仅当 `hasScrollRect` 调 setter |
...
| LayerBuilder 函数 | PAGX 节点类型 | LayerDoc Data |
|---|---|---|
| convertRectangle 309-317 | `pagx::Rectangle` | ElementRectangleData |
| convertEllipse 319-326 | `pagx::Ellipse` | ElementEllipseData |
| convertPolystar 328-344 | `pagx::Polystar` | ElementPolystarData |
| convertPath 360-368 | `pagx::Path + PathData` | ElementShapePathData（已应用 renderScale）|
| convertFill 387-410 | `pagx::Fill` | ElementFillStyleData |
| convertStroke 412-446 | `pagx::Stroke` | ElementStrokeStyleData |
| convertTrimPath 573-582 | `pagx::TrimPath` | ElementTrimPathData |
| convertTextPath 584-602 | `pagx::TextPath` | ElementTextPathData（path 已包含平移）|
| convertRoundCorner 604-608 | `pagx::RoundCorner` | ElementRoundCornerData |
| convertMergePath 610-616 | `pagx::MergePath` | ElementMergePathData |
| convertRepeater 618-630 | `pagx::Repeater` | ElementRepeaterData |
| convertTextModifier 632-679 | `pagx::TextModifier` | ElementTextModifierData |
| convertGroup 681-729 | `pagx::Group / TextBox` | ElementVectorGroupData（renderPosition 已合成）|
| convertText 370-385 + PrepareTextBlob 236-243 | `pagx::Text` | ElementTextData（GlyphRunBlob 原材料）|

### F.3 ColorSource 字段对照

| LayerBuilder 函数 | PAGX 节点 | ShapeStyleData 字段 |
|---|---|---|
| convertLinearGradient 506-514 | `pagx::LinearGradient` | sourceType=LinearGradient, startPoint, endPoint, stopColors, stopPositions, gradientMatrix |
| convertRadialGradient 516-523 | `pagx::RadialGradient` | sourceType=RadialGradient, center, radius, stops, matrix |
| convertConicGradient 525-532 | `pagx::ConicGradient` | sourceType=ConicGradient, center, startAngle, endAngle, stops, matrix |
| convertDiamondGradient 534-541 | `pagx::DiamondGradient` | sourceType=DiamondGradient, center, radius, stops, matrix |
| convertImagePattern 543-571 | `pagx::ImagePattern` | sourceType=ImagePattern, patternImageIndex, tileModeX/Y, filterMode, mipmapMode, patternMatrix |

**Gradient stops 兜底**（对齐 483-496）：Baker 必须在 stops 为空时注入 `[Black@0, White@1]`。
...
| 300-399 | Codec | 致命（doc=nullptr） | InvalidMagic |
...
enum class ErrorCode : uint16_t {
  ...
  EmptyCompositions         = 103,  // 产出的 LayerDoc 无 compositions（不合法状态）
...
struct Diagnostic {
  ErrorCode code = ErrorCode::Ok;
  std::string message;           // 可选的人类补充信息（如 "image at index 3 missing"）
  uint32_t byteOffset = 0;       // 字节流位置快照；见下方"填写规则"
};
...
| **Inflater（LayerInflater::Inflate）** | warning 时 | `0`（操作的是 LayerDoc 对象树，无字节流） |

**便捷构造辅助**（`src/pagx/pag/Diagnostic.h`）：
...
// 用于 Baker/Inflater：byteOffset 为 0
...
void DecodeContext::error(ErrorCode code, std::string msg) {
  errors.push_back({code, std::move(msg),
                    static_cast<uint32_t>(streamContext /* or stream */->position())});
}
void DecodeContext::warn(ErrorCode code, std::string msg) {
  warnings.push_back({code, std::move(msg),
                      static_cast<uint32_t>(stream->position())});
}
```

> **注**：具体实现需要 DecodeContext 持有指向 DecodeStream 的指针，或由调用方传入 offset。建议 DecodeContext 持有弱引用（`pag::DecodeStream* currentStream = nullptr;` public 字段，Codec 主循环在每次 dispatch 前更新），避免每条 error/warn 调用都显式传 offset 带来的样板代码。

**日志/UI 展示建议**：`FormatDiagnostic`（§G.4）的实现里，当 `byteOffset != 0` 时追加 `" @0x<hex>"`；例如：
```
...
```
`byteOffset == 0` 时不追加，保持 Baker 路径的简洁。

**测试影响**：附录 G.6 的错误码矩阵表里，**每条测试**在断言 `result.errors[0].code == ...` 后**可选**追加 `EXPECT_GT(result.errors[0].byteOffset, 0u)`（仅对 Decoder 路径的测试）。不强制。

### G.4 公共 API 转换

PAGExporter::Result 的 `errors/warnings` 是 `vector<string>`，由内部转换器 `std::string FormatDiagnostic(const Diagnostic&)` 把 `Diagnostic` 格式化为 `"[<code-name>] <message>"`（如 `"[LayoutNotApplied] document must have applyLayout() called first"`）。
...
- **MAX_COMPOSITIONS / MAX_LAYERS_PER_COMPOSITION / MAX_VECTOR_ELEMENTS_PER_PAYLOAD / MAX_FILTERS_PER_LAYER / MAX_STYLES_PER_LAYER / MAX_GRADIENT_STOPS / MAX_PATH_VERBS / MAX_GLYPHS_PER_RUN**：Baker 在向 LayerDoc 推入每批次数据前检查，超限推 fatal 错误（选用 `ErrorCode::InvalidDocument`-系或新增对应错误码）。
...
- **MAX_NAME_STRING_BYTES (64 KB)**：Baker 写入 `Layer::name` / `Composition::id` / `FontAsset::family/style` 前截断并推 warning `StringInvalidUtf8`（字段名复用）。
- **MAX_TEXT_STRING_BYTES (10 MB)**：Baker 写入 `TextPayload::text` 或 `ElementText` 相关字段时适用（本期 PAGX 不会产出 TextPayload，但 OutlineAll 模式下 Baker 仍需对原始文本做长度检查，超限截断并 warn）。
...
### I.1 公共头（`include/pagx/PAGExporter.h`）

**严格禁止**包含的头：
- 任何 `tgfx/` 开头的头；
- 任何 `src/` 目录下的头；
- 任何 `pagx/pag/` 目录下的头（LayerDoc / Codec / Baker / Inflater）。

**允许包含**：
- `pagx/PAGXDocument.h`；
- STL 头（`<memory>`, `<string>`, `<vector>`, `<cstdint>`）；
...
target_link_libraries(pagx-pag
  PRIVATE pagx-core tgfx)          # 具体名称以项目 CMakeLists 为准
...
- v2.1 → v2.2 修订要点（深度评审补强）：
  ...
  - **P1-1 LayerDoc 错误路径内存管理**（§8.3bis 新增）。统一用 `std::unique_ptr<LayerDoc>` 管理根对象，fatal 立即 `reset()`，依赖析构链式释放子对象；禁止栈对象/裸 new/shared_ptr 三种反面模式；补充 `BakeResult` 结构定义与 `DecodeResult` 对称；§18.4bis 追加 `BakerMemoryTest.FatalDuringLayerBakeNoLeak` ASAN 测试。
...
- v1.3 → v1.4 修订要点（P0 阻塞修复）：
  ...
  - 补充附录 D.1 `ColorSpace` 处理强约束：`pagx::Color` 含 `colorSpace` 字段（`SRGB` / `DisplayP3`），`tgfx::Color` 和 LayerDoc 字节流**只承载 sRGB**。Baker **必须**通过 `ToTGFX(const pagx::Color&)`（`src/renderer/ToTGFX.cpp:70-85`）完成色域转换，不得手写 `{c.red, c.green, c.blue, c.alpha}` 直接赋值，否则 `DisplayP3` 样本会渲染偏色（E3）。
...
- v1.0 → v1.1 修订要点：
  ...
  - 新增附录 C/D/E/F/G/H/I（LayerDoc 完整定义 / 字节布局 / 枚举映射 / 字段对照 / 错误码 / 安全上限 / 依赖规范）；
...
> 本附录是 LayerDoc 的权威来源。§5 是概览，以下是完整字段表——AI 编码期间按此附录落 `LayerDoc.h`。所有 `Property<T>` 含义见 §5.1；所有 enum 映射见附录 E；所有字节布局见附录 D。
...
// 便携类型别名：LayerDoc 一律使用 tgfx 枚举语义（PAGX→tgfx 在 Baker 一次完成）
...
// 为避免 LayerDoc 依赖 pagx 头文件，在 pagx::pag 命名空间内重新定义一份值对齐的枚举。
...
struct GlyphRunBlob {
  ...
  uint32_t fontIndex = 0;            // 指向 LayerDoc::fonts 的索引；UINT32_MAX 表示字体缺失降级
...
struct ShapeStyleData {
  ...
  // Linear
  Property<Point> startPoint = MakeProp(Point{});
  Property<Point> endPoint   = MakeProp(Point{});
