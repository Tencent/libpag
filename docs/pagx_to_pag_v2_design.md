...
| **4 Context** | `BakeContext`（Baker 用） / `EncodeContext`（Encode 用） / `DecodeContext`（Decode 用） / `InflaterContext`（Inflate 用）——见 §8.5 / §9.4 |
...
- 4 Context 结构**对称但不相同**——EncodeContext 无 errors（Encoder 只产 warnings）、InflaterContext 无字节流（无 byteOffset）、BakeContext 无字节流；共同点是都有 `warnings + MAX_DIAGNOSTICS cap`。
...
struct BakeContext {
  ...
  // 两层去重（P1-7 v2.15）：先 byNode 后 byKey；避免不同 node 指向同一物理图像 / 字体时重复入表。
  std::unordered_map<const void*, uint32_t>   imageIndexByNode;   // Layer 1：PAGX node 指针
  std::unordered_map<std::string, uint32_t>   imageIndexByKey;    // Layer 2：content key（URI / 绝对路径；内嵌用 Data* hex 串）
  std::unordered_map<const void*, uint32_t>   fontIndexByNode;
  std::unordered_map<std::string, uint32_t>   fontIndexByKey;     // "system\0family\0style" 或 "embedded\0DataPtrHex"
  ...
  bool registerComposition() {
    if (totalCompositionCount >= limits::MAX_COMPOSITIONS) {
      error(ErrorCode::StructureLimitExceeded, "MAX_COMPOSITIONS exceeded");
      return false;
    }
        ...
        - 每个 PAGDocument::Layer：
            ...
            * 递归 children：对每个 childPagLayer 调 inflateLayer 得 childTgfxLayer；
              **若 childTgfxLayer 为 nullptr（CompositionRef 进环 / 超 MAX_COMPOSITION_REF_DEPTH /
              MAX_INFLATED_LAYER_COUNT 熔断等降级路径）**，用 `tgfx::Layer::Make()` 生成空壳 Layer
              占位——**保持 layerPath 子索引序号连续**，否则 Baker pre-pass 生成的
              `layerPathByPagxLayer` 与 Inflater 实际 `layerByPath` 错位，Pass 2 Mask 解析会连带
              触发误报的 `InflateMaskResolveFailed=603`。
            * 然后 addChild(childTgfxLayer)
            * 若 maskLayerPath 非空，记入 pendingMasks
            * 把 (layerPath, tgfxLayer) 写入 layerByPath（包括空壳占位 Layer，layerPath 是 Baker
              指派的结构坐标，占位 Layer 仍有合法坐标）
...
PackedLayerPath PackLayerPath(const std::vector<uint32_t>& path);
...
struct InflaterContext {
  ...
  //
  // Resize 契约（P2-3 v2.15）：`visitingComposition` 必须在 Inflate() 入口显式
  // `visitingComposition.assign(doc->compositions.size(), false)`——不在 Decoder 出口 resize；
  // 原因是 Decoder 产出的 PAGDocument 可被多次 Inflate（理论上，Phase 10.5 暂未开放但接口留此自由度），
  // 每次 Inflate 都应重置状态。
...
**去重 key**（两层，P1-7 v2.15，强制）：
- Layer 1（快速）：**PAGX node 指针** → 同一 PAGX Image 节点被多 Layer 引用时 O(1) 命中（现有 `BakeContext::imageIndexByNode`）；
- Layer 2（内容）：**内容签名**，以避免不同 PAGX node 指向同一物理图像时重复入 ImageAsset：
  - 内嵌（`imageNode->data` 非空）：以 `tgfx::Data*` 指针为 key（`tgfx::Data` 一旦构造内容不可变，指针等价内容等价）；
  - data URI：以完整 URI 字符串为 key；
  - 文件路径：以**已解析的绝对路径**字符串为 key（PAGX 导入阶段已 resolve）。

ResourceBaker pre-pass 查表顺序：先 `imageIndexByNode[nodePtr]`，不命中再 `imageIndexByKey[contentKey]`（contentKey 是 `std::variant<const tgfx::Data*, std::string>`），再不命中才 `push_back` 新 ImageAsset。规模估算：图标模板被 50 Layer 共享的场景，从 50× push 降为 1× push + 49× Layer 2 命中，省 ~245 MB（50 × 5 MB PNG）+ 49 次解码。FontAsset 同。

BakeContext 对应新增 `imageIndexByKey` / `fontIndexByKey`——见 §8.5 更新。
...
**去重 key**（两层，P1-7 v2.15）：
- Layer 1（快速）：**PAGX node 指针** → `fontIndexByNode`；
- Layer 2（内容）：
  - System：`(family, style, "")`
  - Embedded：以 `tgfx::Data*` 为内容 key（若字体也已改 shared_ptr<tgfx::Data>；本期 Embedded `data` 仍为 `vector<uint8_t>`——开工首周同步升级到 `shared_ptr<tgfx::Data>` 对齐 ImageAsset，P1-7 配套）
 ...
 *   306 InvalidCompositionIndex  → compositionIndex (raw value read from bytes)
 *   402 ResourceSizeExceeded     → imageIndex OR fontIndex (disambiguated by message prefix)
...
**配套测试** `PAGXBuilderTest.cpp`（Phase 2 交付）：
- `Minimal.Root` / `Composition.NestedRef` / `Layer.Mask.ByPath` / `VectorGroup.Depth` / `Text.BasicRun` / `Image.EmbeddedBytes` / `Build.LayoutApplied` 共 7-10 条。
...
//
// `MakeDeepLayerStack(builder, depth)` 放 `test/src/pag/support/StructBuilders.h`，
// Phase 4 交付（与 Phase 4 TruncatedDecodeTest 同批）。
```
...
| `ValueCodecSafe.Bytes.Oversized` | ReadLengthPrefixedBytes | length > MAX_IMAGE_BYTES → warn ResourceSizeExceeded + 空 + 对齐 skip |
...
| 4 | `Codec.cpp` 基础 Tag（FileHeader/Composition/LayerBlock） | `RoundTripTest` 前半 + `VersionRejectTest` + `TruncatedDecodeTest`（**仅** 300/301/302/303/305/306 段。FileReadFailed=307 只能由 LoadFromFile 触发，推至 Phase 10.5；Path/Glyph/Resource 相关 402/403/404/405 推至 Phase 5/8；CompositionCycle=605 推至 Phase 9） | 全绿 |
...
inline std::vector<uint8_t> ReadLengthPrefixedBytes(pag::DecodeStream* s, DecodeContext* ctx,
                                                   size_t maxBytes) {
  uint32_t n = s->readEncodedUint32();
  if (n > maxBytes || n > s->bytesAvailable()) {
    ctx->warn(ErrorCode::ResourceSizeExceeded, "bytes length exceeds limit or stream");
    s->setPosition(std::min<size_t>(s->position() + std::min<uint64_t>(n, s->bytesAvailable()),
                                    s->size()));   // 尽力 skip 保持对齐
    return {};
  }
  std::vector<uint8_t> out(n);
  s->readBytes(out.data(), n);
  return out;
}
```
...
**DiagnosticTest.cpp 断言清单**（阶段 0 必过，共 9 条）：
...
| `CodeToString.AllEnumValues` | **遍历全部已定义 `DiagnosticCode` 枚举值**（用 `kAllDiagnosticCodes[]` 表 / 手写展开），断言 `CodeToString(c) != nullptr` 且 `string(CodeToString(c)).size() > 0`。保护：新增 enum 值未加 switch case 时 DiagnosticTest 失败（`-Wswitch` 编译期告警 + 运行时断言双重防御）。|
...
// 注：BAKE_MAX_TOTAL_COMPOSITIONS 复用 MAX_COMPOSITIONS=1000，语义一致无需另设。
...
v2.0 → v2.15 累计 6 轮专家评审，~50 个 P0/P1 修复。**开工前只需记下这些硬约束**，其他版本日志作"为什么这样设计"的补充阅读：
...
10. **Phase 出口条件**：§19 阶段表严格按 Tag/功能拆错误码覆盖范围——Phase 4 只覆盖 300/301/302/303/305/306，其他推至对应 Phase；`PAGDecodeFuzz` 目标在 Phase 12 ≥1 CPU·小时 ASAN/UBSAN 全绿。
