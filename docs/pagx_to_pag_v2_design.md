...
升 `FORMAT_VERSION` 之前必须执行下表完整 checklist。本表是版本升级的唯一权威流程，其他章节引用此表。
...
### 4.3 Property<T> 编码（⚠ 唯一权威定义）

> 本节为 `Property<T>` 字节编码与 `propHeader` 位域的**唯一权威来源**。其他章节（§5.2 / §13.1 / 附录 C.2）涉及 Property 的描述均以本节为准，不重复定义。修改 propHeader 格式时只改本节。
...
所有**可动画字段**以 `Property<T>` 包装。字节编码与默认值纪律见 §4.3（权威定义）。**Baker 赋值统一走 `MakeProp(v)` 辅助**，不得手写字面量——这是为动画扩展留的唯一修改点（见 §13）。
...
### 6.1 TagCode 分段（⚠ 唯一 TagCode 注册表）

> 本节为 TagCode 编号空间的**唯一权威来源**。所有新增 Tag 必须在此表登记。
>
...
### G.2 错误码枚举（⚠ 唯一真相源 + 权威定义在 `include/pagx/Diagnostic.h`）

**本节为错误码的唯一权威索引**。枚举值的权威声明在对外公共头 `include/pagx/Diagnostic.h`（§15.1 内嵌代码），内部通过 `using ErrorCode = pagx::DiagnosticCode;` 别名访问。其他章节（§G.6 测试矩阵、§19 阶段拆分）对错误码的引用均以本节为准，不重复列举。
