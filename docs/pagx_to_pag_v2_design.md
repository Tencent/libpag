...
7. **文本 runtime-shape**（Phase 16 v2.20）：`.pag` 文件只存原始 text + fontFamily/fontStyle，Inflater 端用 `TextShaper` 运行时 shape；字体由调用方通过 `FontProvider`（或全局 `pag::FontManager`）在播放端注册。与 v1 行为对齐，规避跨平台 glyph ID 不稳定性。原 v2.0-v2.19 规划的"Render / OutlineAll 双模式"中 OutlineAll 分支 v2.20 起已从 API 与测试框架移除（未来需要时再新加）。
...
[编码]
  *.pagx (XML)
  ...
  PAGDocument
    │
    ▼ pagx::pag::Codec::Encode       [EncodeContext]     ◀── v2.20 P1-23：从裸指针聚合 EncodeSession 升级为独立 Context（DiagnosticCollector 子类），与其他 3 Context 对称
  *.pag (version = 0x02)
...
| **4 Context 对称** | `EncodeContext` / `BakeContext` / `DecodeContext` / `InflaterContext`——均继承 `DiagnosticCollector`（protected pushError/pushWarning helper + 子类 3 参 public error/warn wrapper）；EncodeContext / InflaterContext 无 fatal，BakeContext / DecodeContext 有 `errors` + `hasError()`——见 §8.5 / §9.4 |
...
- 4 Context 共享 `DiagnosticCollector` 基类提供的 MAX_DIAGNOSTICS cap 与 message 截断；具体 API 定义见 §8.5。DecodeContext 另持 `currentStream` 在 wrapper 内自动填 byteOffset；EncodeContext / BakeContext / InflaterContext 的 byteOffset 恒 0；EncodeContext / InflaterContext 不持有 `errors`（无 fatal 语义）。
- **阅读链**：先读本 §3.2 全览表 → 再读"文档维护"节的"上次开工必读 17 条" → 编码期按需查 §3.3 术语索引。
...
| **Context / Session** | | |
| `BakeContext` / `DecodeContext` / `InflaterContext` / `EncodeContext` | 四个阶段的 Context 对象（状态 + 诊断收集，均继承 DiagnosticCollector） | §8.5 / §9.4 |
...
| ~~`GlyphRunBlob`~~ / ~~`FontAsset`~~ | v2.0-v2.19 的字体预 shape 数据结构，**Phase 16 v2.20 废弃** | §10 / §11.2 / §C.4 |
...
> **TagCode 分段沿革**：Filter 段从 v1 的 70-74 迁至 80-99 再迁至 120-139；Style 段从 v1 的 80-82 迁至 100-119 再迁至 140-159；VectorElement 段从 40-79 扩至 40-119（为 3D 及 AE modifier 预留深度池）。**240-299 段（原"资源扩展"预留）v2.20 起删除**：当前 v2 没有 SVG / Lottie / HDR 资源的 roadmap，TagCode 空间几乎无成本但文档化的"预留席位"会暗示"即将支持"——按 §2 精简原则直接去掉；真正需要时按 §6.5 字段级追加规则新划段即可。v2 未发布，纯数值重分段零代码成本。附录 D.11/D.12 TagCode 值以当前表为准。
...
```
Phase 16 (v2.20):    FileHeader → ImageAssetTable → CompositionList → End
v2.0-v2.19 (legacy): FileHeader → ImageAssetTable → FontAssetTable → CompositionList → End
```

> **Phase 16 v2.20 变更**：`FontAssetTable` 从写入序列中移除——字体信息改由 `ElementTextData::fontFamily/fontStyle` 直接携带（§10、§C.7、§D.11）。Decoder 遇到老版本开发分支产出的 `FontAssetTable` 字节段按 `UnknownTagCode=400` warn + length skip，不崩溃。
...
**必选 top-level Tag 清单**（强制）：Phase 16 v2.20 起为 **FileHeader / ImageAssetTable / CompositionList / Composition / LayerBlock 五个 Tag 必选**——缺失任何一个 = 丢失 canvas 尺寸 / 资源池 / composition 列表 / 整 composition / 整 Layer 子树 = 整文件或整 composition 不可播。这五个 Tag 的**字段语义不兼容变更**必须升 FORMAT_VERSION（②），只能走 ① 字段级追加；其他 Tag 属功能性 Tag，字段不兼容变更同样走 ②（避免双写复杂度）。
>
> v2.0-v2.19 曾把 `FontAssetTable` 也列为必选，Phase 16 回退 runtime-shape 后 Encoder 不再产出；老 .pag 字节流若含 `FontAssetTable` 走 `UnknownTagCode=400` warn + length skip，不构成阻塞（见 §6.2 / §11.2）。
...
| **TextBaker** | `layer_bakers/TextBaker.cpp` | Text/TextBox/TextPath（Phase 16 v2.20 runtime-shape 模式——把 pagx::Text 的 text/fontFamily/fontStyle/排版字段展平到 ElementTextData，Inflater 端调 TextShaper） | 236-260, 370-385, 584-602, 296-303 |
...
- Mask 采用两趟策略（见 §12）。
- **输入 PAGXDocument 的生命周期约束**（P0-18 v2.20，强制）：`Baker::Bake(const PAGXDocument& doc)` 在整个调用期间（包括两趟 pre-pass + main pass）**禁止**由任何线程修改 `doc` 的结构或回收其节点。`BakeContext::imageIndexByNode` / `layerPathByPagxLayer` 等索引以 `const void*`（PAGX 节点原生指针）为 key——一旦节点被移动、去重合并或 GC，key 即失效，后续查表命中错误节点或 UAF。调用方责任：在 `Bake` 返回前，`doc` 作为只读对象持有，且不得触发任何结构重整操作（例如 `applyLayout`、DOM 编辑、内存池压缩）。多线程场景下，调用方必须在 `Bake` 返回前保证读者-写者互斥。
...
class Codec {
 public:
  // Encode 内部构造 `EncodeContext ctx` 栈对象（§8.5）——
...
# Phase 16 v2.20：原 WriteTag(body, FontAssetTable, doc.fonts) 已移除。
# 字体信息改由 ElementTextData 直接携带（§10 / §D.11）。
...
**预估公式**（`EncodeStream::estimateBodySize(doc)`）：根据 §18.8 基线 `size_ratio ≈ 0.3` 反推，`bodyBytes ≈ pagxTotalDataBytes × 0.4`（留 30% 余量）。实现放 `src/pagx/pag/EstimateSize.h`。若无 PAGX 侧的 `totalDataBytes` 提示，退化为累加 `doc.images[i].data->size() + 4 KB × layerCount`（Phase 16 v2.20 起 `doc.fonts[]` 不存在，原 font bytes 累加项移除；字体信息改由 ElementTextData 的 fontFamily/fontStyle 字符串直接携带，字节体积可忽略）。50 MB 大工程文件上 encode 时间从 log2(50 MB) ≈ 26 次 realloc + memcpy 降到 1-2 次 ≈ 省 30% encode_ms。

**Encode 深度防线退化说明**（P1-04 v2.19 + P1-23 v2.20）：v2.17 及之前，Encode 阶段通过 `EncodeContext::currentLayerDepth` / `currentVectorElementDepth` 字段在 Write Layer/VectorGroup 时重复校验一次深度（二道防线）。v2.18 P0-2 合并 EncodeContext 字段、v2.19 P0-D 改为 EncodeSession 聚合、v2.20 P1-23 恢复独立 EncodeContext——Encode 侧始终不再记账深度——信任 Baker 已做 MAX_LAYER_DEPTH / MAX_VECTOR_ELEMENT_DEPTH 校验（§H.2 BakeContext::enterLayer/enterVectorGroup）。若 Baker 单测覆盖不足导致 PAGDocument 结构深度异常进入 Encode，将在 **Decode 侧** 由 DecodeContext::currentLayerDepth 按 MAX_LAYER_DEPTH 反拦推 `LayerDepthLimitExceeded=406` fatal（或 Inflater Pass 1 拒绝 inflateComposition），作为回归测试兜底——不会让畸形字节流污染下游 Inflater。
...
- Decoder 读到 `ErrorMarker` 立即按 **三档语义**（P1-18 v2.19 细化，按 ErrorMarker 出现的字节流位置分派）：
  - **档 (i)** fatal 发生在 top-level Tag 之间（FileHeader/ImageAssetTable/CompositionList 等顶层序列中；Phase 16 v2.20 起 FontAssetTable 已从顶层序列移除）：Decoder 推 `ProducerFatal=106` error + **直接 return**（无法部分恢复——缺失 images/compositions 任一将导致下游 Inflater 索引越界，丢全文件是唯一安全选项）。
...
### 8.5 DiagnosticCollector / EncodeContext / DecodeContext / BakeContext / InflaterContext

**架构基线**（v2.20 P1-23 改版）：

- **DiagnosticCollector 基类**：4 个 Context（EncodeContext / BakeContext / DecodeContext / InflaterContext）共享 `warnings` vector + MAX_DIAGNOSTICS cap + MAX_DIAGNOSTIC_MESSAGE_BYTES 截断逻辑，抽取为 `struct DiagnosticCollector`，**仅暴露 protected helper** `pushError/pushWarning`。子类各自定义 **3 参 public wrapper** `error(code, msg, contextIndex)` / `warn(...)`。
- **`hasError()` 归子类**：EncodeContext / InflaterContext "无 fatal" 语义——`errors` 字段 + `hasError()` 方法仅声明在 BakeContext / DecodeContext 侧，不下沉基类。
- **EncodeContext 与其他 Context 对称**（v2.20）：从 v2.19 的裸指针聚合体 `EncodeSession` 升级为独立 Context，持有 `pag::StreamContext` + `debugLayout`。**动机**：保持 4 个 Context 形态一致（都继承 DiagnosticCollector，都有 3 参 `warn`），避免调用点在 "`session->diag->pushWarning(...)`" 与 "`ctx->warn(...)`" 两种风格间切换；Encode 仍无 fatal（不 override `pushError`）。所有 `Write<TagName>` 签名第三参为 `EncodeContext* ctx`。

> 架构心智：**4 Context 对称**——BakeContext / DecodeContext 持 errors + warnings；EncodeContext / InflaterContext 仅持 warnings（无 fatal）；共用基类 DiagnosticCollector 统一 cap 逻辑。
...
struct DiagnosticCollector {
 ...
 protected:
  ...
  // EncodeContext / InflaterContext **不 override**——物理屏蔽 "无 fatal" 语义。
  virtual void pushError(Diagnostic d) { /* no-op default */ }

  virtual ~DiagnosticCollector() = default;
};

// ---------- EncodeContext（v2.20 P1-23；替代 v2.19 EncodeSession 聚合） ----------

struct EncodeContext : DiagnosticCollector {
  pag::StreamContext streamContext;          // 传给 v1 EncodeStream 构造

  // Debug-only：仅 `PAGX_DEBUG_OFFSETS && PAG_BUILD_TESTS` 构建下非空；用于 PAGXBuilder /
  // TestUtils 通过 §18.3bis CapturePagxLayout 记录关键 Tag 字节偏移。release build 恒 nullptr。
  PagxBytesLayout* debugLayout = nullptr;

  void warn(ErrorCode code, std::string msg = {}, uint32_t contextIndex = UINT32_MAX) {
    pushWarning({code, std::move(msg), 0, contextIndex});   // Encode 无字节流 position 语义，byteOffset=0
  }
};

// EncodeContext 典型使用（Codec::Encode 栈内构造 + 向下传递）：
//
//   EncodeResult Codec::Encode(const PAGDocument& doc) {
//     EncodeContext ctx;                         // Encode 只产 warnings，无 fatal
//     pag::EncodeStream body(&ctx.streamContext);
//     body.reserve(estimateBodySize(doc));
//
//     WriteFileHeader(&body, doc, &ctx);
//     WriteImageAssetTable(&body, doc.images, &ctx);
//     for (const auto& comp : doc.compositions) {
//       WriteComposition(&body, *comp, &ctx);
//     }
//     WriteEndTag(&body);
//
//     EncodeResult r;
...
//   }
//
// 每个 `Write<TagName>(EncodeStream*, const Payload&, EncodeContext* ctx)` 内部
// 通过 `ctx->warn(...)` 推诊断，不涉及 fatal（Encode 无 fatal 路径）。
...
- Encode 侧同理：v1 `EncodeStream` 不会产生错误（只增长 buffer），`EncodeContext::streamContext` 作为 stream 构造参数传入，但不需要每 tag sync（§8.5 v2.20 P1-23）。
...
struct BakeContext : DiagnosticCollector {
  ...
  // 资源索引化（§7.2 pre-pass 结果；VectorBaker 查表用）
  ...
  // Phase 16 v2.20 移除：fontIndexByNode / fontIndexByDataPtr / fontIndexByKey
  // 原 pre-pass 为 FontAsset 生成 fontIndex；runtime-shape 模式下字体信息直接嵌入
  // ElementTextData::fontFamily/fontStyle，不再需要资源索引。TextBaker 不再查 fontIndex。
...
**Context 总览（v2.20 P1-23：4 Context 对称）**：

| Context | 用途 | errors | warnings | byteOffset | MAX_DIAGNOSTICS cap |
|---|---|---|---|---|---|
| `EncodeContext : DiagnosticCollector` | 字节流编码 | —（Encoder 不产 error） | ✓ | 恒 0 | ✓ |
...
| `InflateFontCreateFailed` (601) | Phase 16 v2.20：`inflateElementText` 内 `FontProvider::getTypeface(fontFamily, fontStyle)` 返回 null 且 fallback 链也无可用 typeface | 该 ElementText 渲染为空白占位；v1 v2.0-v2.19 原触发路径 `resolveFontAsset::MakeFromBytes` 已随 FontAsset 废弃而作废 |
| `InflateGlyphRunBuildFailed` (602) | Phase 16 v2.20：`inflateElementText` 内 `TextShaper::Shape` 返回空 glyph 列表或段落布局产生 null TextBlob | 该 ElementText 被跳过，其外层 VectorGroup 其他子元素仍渲染；v2.0-v2.19 原触发路径 `GlyphRunRenderer::BuildTextBlob*` 已随 GlyphRunBlob 废弃而作废 |
...
> **历史**：v2.0 – v2.19 曾规划"字体双模式（Render / OutlineAll）" + GlyphRun 预 shape 序列化。Phase 15 实测发现架构级缺陷（`tgfx::SystemFont::MakeFromName` 在 macOS/Linux/Android 硬返回 nullptr，预存的 glyph ID 无 typeface 可匹配，文本全空白）。**v2.20 Phase 16 回退为"v1 风格 runtime-shape"**——存原始文本串 + 字体名，Inflater 端动态 shape。完整回退设计见 [`pagx_to_pag_v2_phase16_text_redesign.md`](pagx_to_pag_v2_phase16_text_redesign.md)。
...
- **Inflater 重建**：`TextShaper::Shape(text, typeface)` 动态 shape + 复用 v1 TextLayout 做段落布局（justification / box wrap / tracking）。
...
不同于 v2.0-v2.19 的 GlyphRunBlob 路径，Phase 16 的 `ElementTextData` 直接镜像 v1 `pag::TextDocument` 字段集（参见 `include/pag/types.h:class PAG_API TextDocument`）。权威 C++ 结构见 §C.7；字节布局见 §D.11。
...
**兼容性**：ElementText payload 从 GlyphRunBlob schema 改为 TextDocument-风格字段集属**字段语义不兼容变更**，按 §4.1.1 / §6.5 规则应升 FORMAT_VERSION；但 PAG v2 尚未对外发布（仅在本仓内使用），事实层面不构成任何用户 .pag 文件破坏，故**不升 FORMAT_VERSION，继续 0x02**。老版本开发分支产出的 .pag 文件（含 GlyphRunBlob 路径）不再被新 Reader 识别——会在 ElementText sub-Tag 解析期发 `MalformedTag=304` 并 skip 整个 Text element，不崩溃。
...
2. Baker 把每个 span 的 `inverseMatrix` 展平为 `ElementTextData::position / anchors` 的后乘修正（而不是序列化 Matrix 本体）。
3. Inflater 端 shape + 段落布局完成后得到的 TextBlob 已经在 Text 自身的本地坐标系，外层 VectorGroup 的 position/anchor/scale 再叠加——与 v1 TextRenderer 一致。

### 10.5 OutlineAll 模式（v2.20 起已移除）

原 Phase 8 规划的 "字体转 Path" 管线（`FontMode::OutlineAll`）依赖 Baker 端持有 typeface + glyph outline 提取能力，在 Phase 15 实测阶段被发现与 runtime-shape 路径互相制约——既不能共享 TextBaker 代码路径又没有清晰的验收场景，**v2.20 起从公开 API 与测试框架中彻底移除**（`PAGExporter::Options::FontMode` 枚举、`--pag-font-mode` CLI 选项、`PAGRenderEquivalenceTest::OutlineAll_Baseline` 参数化 test 全部删除，避免长期 SKIP 腐化）。若未来确有需求，按 §6.5 字段级追加规则新增独立选项即可，不需要在当前文档里留"坑位"。
...
| ~~FontAsset~~ | ~~废弃~~ | — | — | — | Phase 16 废弃，详见 §11.2 |

**去重原则**：同内容多次引用只入表一次；VectorElement 通过 `uint32_t imageAssetIndex` 引用。详细字段定义见 §11.1；索引化流程见 §11.3。字体在 Phase 16 runtime-shape 模式下由 `ElementTextData::fontFamily / fontStyle` 字符串直接携带，不再有独立资源池（§10 / §11.2）。
...
ResourceBaker pre-pass 查表顺序：先 `imageIndexByNode[nodePtr]`，不命中依据 PAGX Image 来源分支——内嵌走 `imageIndexByDataPtr`，URI/文件走 `imageIndexByKey`——任一命中即复用 index，全 miss 才 `push_back` 新 ImageAsset。规模估算：图标模板被 50 Layer 共享的场景，从 50× push 降为 1× push + 49× Layer 2 命中，省 ~245 MB（50 × 5 MB PNG）+ 49 次解码。（Phase 16 v2.20 废弃：原 FontAsset 也走同样的三层去重，现已随 §11.2 整节废弃。）
...
### 11.2 FontAsset（Phase 16 废弃）

> **v2.20 Phase 16 起整节废弃**。原设计把 PAGX Font 节点中的 TTF bytes / 系统字体名索引化到 `PAGDocument::fonts[]` + 从 `GlyphRunBlob.fontIndex` 引用。Phase 16 改为运行时 shape（§10），`ElementTextData` 直接携带 `fontFamily` / `fontStyle` 字符串，字体由 Inflater 端 `FontProvider` 查询——**`PAGDocument::fonts[]` 向量、`BakeContext::fontIndexByNode`、`ResourceBaker::indexFonts` pre-pass 全部移除**。`FontAsset` / `FontAxis` / `FontSourceKind` 结构体与 §C.4 一并废弃。
>
> `FontAssetTable = 3` / `FontAsset = 7` TagCode 保留在 §6.1 分段表中**不重用**（符合 §6.5 向前兼容原则），Encoder 不写，Decoder 若读到（例如老版本开发分支产物）走 `UnknownTagCode=400` warn + skip。
>
> 具体字节级变动见 §D.6 / §D.11 ElementText；TextShaper / FontProvider 的对接契约见 §10 及 [`pagx_to_pag_v2_phase16_text_redesign.md`](pagx_to_pag_v2_phase16_text_redesign.md)。

### 11.3 索引化时机

Resource pre-pass 在 `LayerBaker::bake` 之前完成：
1. 深度遍历 `doc.layers`，收集所有 Image node（**Font node 不再收集**，Phase 16 runtime-shape 模式下 Baker 不产出 FontAsset）；
2. 按去重 key 分配 index，写入 `BakeContext::imageIndexByNode`；
3. VectorBaker 在 main pass 中直接查 index 即可，不需回填。TextBaker 则直接把 PAGX Text 节点的 `fontFamily` / `fontStyle` 展平写入 `ElementTextData`（§10.3）。
...
- **Options 初始化风格**：推荐 C++20 designated initializers（`Options{.strict=true}`）而非位置聚合（`Options{true /*strict*/, <other>}`）——位置聚合会把字段序冻死为 ABI，未来添加新字段时老代码错位赋值。C++ 标准低于 C++20 时改用一次一行 `Options opts; opts.strict = true;`。
...
enum class DiagnosticCode : uint16_t {
  ...
  FontResourceSizeExceeded   = 408,   // Phase 16 v2.20 不再触发（FontAsset 废弃）；编号保留不重用（contextIndex = UINT32_MAX）
 ...
 *   201 FontSourceMissing          → layerIndex (the ElementText holder). Phase 16 (v2.20)
 *                                     semantic change: since PAGDocument::fonts[] no longer
 *                                     exists, contextIndex points at the owning layer instead.
 ...
 *   208 TextGlyphRunsDowngraded    → layerIndex (Phase 16 v2.20 new semantic; numeric value
 *                                     unchanged from pre-v2.20 GlyphRunKindInferred)
 ...
 *   408 FontResourceSizeExceeded   → UINT32_MAX (Phase 16 v2.20: no longer triggered;
 *                                     code reserved for backward compatibility only)
 *   600 InflateImageDecodeFailed   → imageIndex
 *   601 InflateFontCreateFailed    → layerIndex (the ElementText holder). Phase 16 (v2.20)
 *                                     semantic change: since PAGDocument::fonts[] no longer
 *                                     exists, contextIndex points at the owning layer instead.
...
class PAGExporter {
 public:
  ...
  // Phase 16 v2.20 起移除 `FontMode` 枚举：runtime-shape 是唯一输出路径（见 §10）。
  // 历史 FontMode::OutlineAll 从未落地，删除以遵循 §2 精简原则——未来真要做 glyph-outline
  // 路径时再按 §6.5 字段级追加规则重新引入新选项（单次方向性决策，不需要提前留 API 坑位）。
...
**Options 初始化风格纪律**（P2-6 v2.15）：推荐使用 C++20 designated initializers（`Options{.strict=true}`）而非位置聚合（`Options{true /*strict*/, <other>}`）。位置聚合会把字段**序**冻死为 ABI——未来添加新字段时任何老代码 `Options{a, b}` 都会错位赋值。若项目 C++ 标准低于 C++20，改用一次一行 `Options opts; opts.strict = true;` 风格，仍避免位置依赖。
...
#include <vector>
#include "pagx/Diagnostic.h"
#include "pagx/pag/FontProvider.h"  // Phase 16 v2.20: for Options::fontProvider
...
class PAGLoader {
 public:
  struct Options {
    ...
    /**
     * Font resolver for text inflation. If null, uses MakeDefaultFontProvider()
     * which wraps the process-global pag::FontManager (requires callers to
     * have pre-registered typefaces via pag::PAGFont::RegisterFont /
     * SetFallbackFontPaths). Phase 16 v2.20 new (see §10 / FontProvider.h).
     *
     * WHY injectable: tgfx::Typeface::MakeFromName returns null on macOS /
     * Linux / Android (only Windows has the DirectWrite backend); PAG v2 must
     * therefore rely on runtime-registered typefaces, and different hosts
     * have different registration mechanisms (PAGFont in libpag v1, custom
     * in iOS/Android SDK, etc.). FontProvider is the single indirection layer.
     */
  ...
  struct Result {
    ...
    int canvasHeight = 0;
    /**
     * Number of compositions in the decoded document (0 if decode failed before
     * the CompositionList was parsed). Populated from `PAGDocument::compositions.size()`.
     *
     * WHY exposed (P0-19 v2.20): several Diagnostic codes carry a `contextIndex`
     * that is the RAW, UNVALIDATED composition/layer index read from the byte
     * stream — notably `InvalidCompositionIndex` (306). Consumers that branch
     * on `contextIndex` must range-check it against the actual composition
     * count; exposing the count here lets them do so without reaching into
     * internal state. See §G.3 and §15.1 Diagnostic ABI table for the
     * per-code contextIndex semantics.
     */
...
Result PAGLoader::LoadFromBytes(const uint8_t* data, size_t length) {
  ...
  out.compositionCount = static_cast<uint32_t>(decoded.doc->compositions.size());
...
└── pag/
    └── FontProvider.h            # Phase 16 v2.20 新增：Inflater 字体解析注入接口（虚类 + MakeDefaultFontProvider 工厂）
...
├── DiagnosticCollector.h         # v2.19 P1-13 + v2.20 P1-23：4 Context 共享基类（protected pushError/pushWarning helper；见 §8.5）
├── EncodeContext.h               # v2.20 P1-23：Encode 阶段 Context（继承 DiagnosticCollector + pag::StreamContext + debugLayout；见 §8.5）
...
│   ├── TextBaker.cpp             # Phase 16 v2.20 起：runtime-shape 模式（把 pagx::Text 字段展平到 ElementTextData，不 pre-shape）
│   ├── StyleFilterBaker.cpp      # 3 style + 5 filter
│   └── ResourceBaker.cpp         # 仅 Image 收集去重（Phase 16 起 Font 相关逻辑移除）
...
│   └── ResourceTags.cpp          # ImageAssetTable（FontAssetTable 在 Phase 16 v2.20 起 Encoder 不产出，Decoder 走 UnknownTagCode warn skip）
...
│   ├── GlyphRunBlobCodecTest.cpp    # Phase 16 v2.20 内容全部标 DISABLED；Phase 16.3 合入后删除
...
2. 项目已有完善的 `Baseline::Compare(tgfx::Surface, key)` 基础设施（基准 webp 比较 + version.json 托管），直接复用减少新增代码。RenderUtil 产出 `std::shared_ptr<tgfx::Surface>`，Baseline::Compare 直接吃（接口定义见 `test/src/utils/Baseline.h`）。
...
| D3 | 单测覆盖率（P1-22 v2.20 分模块） | Baker（`src/pagx/pag/layer_bakers/`）/ Codec（`src/pagx/pag/Codec.cpp` + `tags/`）/ Inflater（`src/pagx/pag/LayerInflater.cpp`）三大模块各 ≥ 80%；整体（所有 `src/pagx/pag/*`）≥ 75%。工具链、辅助头（DiagBuild.h / ValueCodec.h 等）不单列指标。Phase 15 `tools/coverage.sh` 产出 HTML 报告附 PR |
...
- `hasText` 标记供后续筛选使用（当前用于 §18.9 样本特性矩阵生成；无"仅文本样本"专属测试）。
...
class TagWriterScope {
 public:
  TagWriterScope(EncodeStream* s, TagCode code, EncodeContext* ctx)
    ...
    auto* debugLayout = ctx_ ? ctx_->debugLayout : nullptr;   // EncodeContext 额外携带的调试层指针；release build 恒 nullptr
...
**Corpus 初始种子规范**（P0-21 v2.20）：
- **来源**：Phase 1 交付的 `CorruptBuilder` 产物（§18.3bis）是 Phase 12 Fuzz 的**唯一初始种子来源**，不依赖外部样本库。
- **最低种子数**：Phase 12 开启 fuzz 前，`test/fuzz_corpus/seeds/` 必须至少含 **30 条**覆盖 §G.2 中 Codec 400 段与 Inflater 600 段错误码路径（每码 ≥ 1 条；构造脚本位于 `tools/generate_fuzz_seeds.cpp`，编译时走 `PAG_BUILD_TESTS=ON` 链接 CorruptBuilder + StructBuilders）。
- **Corpus 容量上限**：本地 `test/fuzz_corpus/` 不超 **100 MB**；超限时优先剔除 libFuzzer 标记低价值（`feat` 未增长）的样本，不删 regression 种子。CI 上的 cache 目录另由 §CI 工程化规划 5GB 策略管理，两者独立。
- **与单元测试的界线**：CorruptBuilder 产物同时服务于 `TruncatedDecodeTest` 等确定性 regression 测（一对一断言错误码）和 fuzz 初始 corpus（变异基底）；两用途共享生成代码，**禁止**在 fuzz 侧复制一套独立生成器，以免两边漂移。
...
| `GlyphRunBlobCodecTest.cpp` | ~~`GlyphRunBlob.LayoutRunRoundTrip`~~ (Phase 16 v2.20 DISABLED) | 量化编码已随 GlyphRunBlob 结构废弃；Phase 16.3 合入后整个测试文件删除 |
| | ~~`GlyphRunBlob.ClassicGlyphRunRoundTrip`~~ (DISABLED) | 同上 |
| | ~~`GlyphRunBlob.EmptyGlyphCount`~~ (DISABLED) | 同上 |
...
### 18.4quater PackLayerPath 专项测试（P1-11 v2.15 + P1-24 v2.20，`BakeContextTest.cpp` 内 1 条参数化，Phase 3 交付）

- `PackLayerPath.RoundTripMatrix`：参数化 3 组测例合一——`(input={1,2,3}, valid=true)` 正常打包解包；`(input={1,2,3,4,5,6,7}, valid=true)` >6 层退化为"低 6 级 + depth 唯一化"；`(input={1024}, valid=false)` childIndex 超 1023 触发约定行为（断言 / 静默截断为 1023，择一实现）。避免 3 条只差参数的 micro-test 产生阅读噪声（v2.20 从原 3 条 `BasicRoundTrip` / `DepthOverflow` / `ChildIndexCap` 合并而来）。

### 18.4quinta ZeroCopyScope 专项测试（P1-12 v2.18 + P1-24 v2.20，`ValueCodecSafeTest.cpp` 追加 2 条）

- `ZeroCopyScope.OwnershipMatrix`：参数化 2 组合并——`(inScope=false, expected=Adopted)` 默认路径必须走 `MakeAdopted`；`(inScope=true, expected=WithoutCopy)` 作用域内返回 zero-copy Data，`data->data() == stream.currentReadablePtr() + <dataLen prefix>`（原 `DefaultIsOwning` + `InScopeMakesWithoutCopy` 合并）。
- `ZeroCopyScope.LeakDetection`（v2.20 P1-24 双策略定稿，原 `LeakingDataToTreeIsBlocked`）：构造"`MakeWithoutCopy` 产物逃逸到 `PAGDocument::images[0]->data`"的反例 + PAGLoader 释放 backing buffer 后调 Inflater 解码。两种断言路径编译期切换：
  - **ASAN build**（`PAGX_BUILD_WITH_ASAN` 定义）：`EXPECT_DEATH_IF_SUPPORTED(trigger_flow(), ".*heap-use-after-free.*")`；
  - **非 ASAN build**：转用 `weak_ptr<const tgfx::Data>` 生命周期追踪——`weak_ptr<const tgfx::Data> observer = data; /* release backing buffer */ ASSERT_TRUE(observer.expired());` 若 Inflater 阶段 `observer.expired() == false` 说明产物未被释放（非预期，反例失败）。**非 ASAN 环境不再 GTEST_SKIP**——双策略保证所有构建上都能捕获泄漏。
  - **禁止**使用 `testing::GTEST_FLAG(death_test_style)` / `EXPECT_NONFATAL_FAILURE` 替代路径——多写法并存导致 CI 稳定性低。
...
- Baseline 机制会自动把当前产出保存到 `test/out/PAGRenderTest_Render/{sample}.webp`（参考 `test/src/utils/Baseline.cpp` 的 OUT_ROOT 机制）；
...
| 1 | `ValueCodec.h` / `PropertyEncoding.h` / `TagCode.h` / TagHeader util + `test/src/pag/support/CorruptBuilder.h+cpp`（§18.3bis，**Phase 12 Fuzz 初始 corpus 源头，P0-21 v2.20**）| `ValueCodecTest` / `PropertyEncodingTest` / `TagHeaderTest` / `CorruptBuilderTest`（断言每个 fluent setter 产生的字节跟预期 offset/value 对齐，≥ 10 条） | 四文件全绿；CorruptBuilder 本阶段交付以便后续 Phase 4 用；**Phase 1 exit gate**（P1-4 v2.17）：`grep -rn "std::vector<uint8_t>" src/pagx/pag/ValueCodec.h src/pagx/pag/Codec.cpp` 在 `ReadLengthPrefixedBytes` / `ReadUtf8String` / ImageAsset / FontAsset 上下文零命中（全部改用 `std::shared_ptr<const tgfx::Data>`）+ **varU32 安全预检已完成**（§I.5 P0-20 v2.20，结论写入 Phase 1 PR 描述）|
...
| 4 | `Codec.cpp` 基础 Tag（FileHeader/Composition/LayerBlock）——**产出 `src/pagx/pag/EncodeContext.h`**（v2.20 P1-23）：Encode 在 Codec.cpp 函数栈内构造 `EncodeContext ctx;` 临时对象；`EncodeContext` 继承 `DiagnosticCollector`（Phase 0 交付，`include/pagx/pag/DiagnosticCollector.h`），与其他 3 Context API 对称（3 参 `warn`）。 | `RoundTripTest` 前半 + `VersionRejectTest` + `TruncatedDecodeTest`（**仅** 300/301/302/303/304/305/306 段。FileReadFailed=307 只能由 LoadFromFile 触发，推至 Phase 10.5；Path/Glyph/Resource 相关 402/403/404/405/408 推至 Phase 5/8；CompositionCycle=605 推至 Phase 9；LayerBudgetExceeded=606 推至 Phase 9） | 全绿 |
...
| 12 | **`.github/workflows/pagx-fuzz.yml`**（P1-6 v2.17 登记：§18.3ter CI 规划正式落地产物，4-shard matrix × 6h + corpus cache + crash auto-commit） | `RenderEquivalenceTest` + `PAGDecodeFuzzTest`（§18.3ter Layer 6 Fuzz，≥ 1 CPU·小时 ASAN/UBSAN 全绿） | Render 模式 Baseline::Compare 全绿；Fuzz 全绿；CI yaml 首次上绿 |
...
| **16.4** | `LayerInflater.cpp::inflateElementText` 重写——`FontProvider.getTypeface` + `TextShaper::Shape` + v1 `TextLayout` 段落布局 + `tgfx::Text::Make(blob, anchors)` | `LayerInflaterTest` / `LayerInflaterParityTest` 新版 + 新增 `TextInflateShapingTest`（pagx fixtures → inflate → 断言非空 TextBlob） | 全绿 |
| **16.5** | 测试基础设施：`RenderEquivalenceTest` / `PAGRenderCrossCheckTest` 注入 FontProvider；PAGXTest.SpecSamples 用的系统字体注册逻辑下沉到 `test/src/utils/FontRegistration.h` shared util | — | 48 个 RenderEquivalence 含文字样本 inflate 后带非空 TextBlob |
| **16.6** | — | Phase 11.6 遗留的 CrossCheck 文字类 FAIL（≥ 8 个）转绿；`./tools/coverage.sh` 重跑 `LayerInflater/TextBaker` 覆盖率 ≥ 75% | 用户 `/accept-baseline` 接受新基线 |
...
- 阶段 12–15 全绿 + D 类软指标达标 → 可合入 main（M4 里程碑）
- 阶段 16 全绿 → M5 里程碑达成（文本 runtime-shape 回退落地，48 RenderEquivalence 样本含文字部分 inflate 后非空）
...
| 16.4 | Phase 16.3, v1 TextShaper + FontManager | 新 `inflateElementText` |
| 16.5 | Phase 16.4, Phase 12 (RenderEquivalenceTest fixture) | FontProvider 注入 |
| 16.6 | Phase 16.5 | 覆盖率 + baseline accept |
...
- **Phase 4b**：ImageAssetTable + Composition + LayerBlock 主体（v2.0-v2.19 同时包含 FontAssetTable，Phase 16 v2.20 起此表移除）

**Phase 12 首次提交基线缺失说明**（P1-10 v2.18）：`RenderEquivalenceTest` 首次跑时 `test/baseline/PAGRenderTest_Render/` 目录**根本不存在**——48 个样本全部首跑 FAIL 为预期。交付判定应分两步：(a) tech lead 人工审图通过（可用 `tools/render_compare.py` 生成两列对比 HTML）；(b) 用户执行 `/accept-baseline` 批量接受基线，再回跑转绿。AI 遇首跑 FAIL 不应误判为 bug，而应按此流程处理。
...
| 资源索引化 | —（LayerBuilder 现场解码） | `ResourceBaker`（pre-pass，Phase 16 起仅 Image） | `InflaterContext::buildImages` |
...
1. ~~**GlyphRun 序列化最小字段集**：已摸清，定稿见附录 C 的 `GlyphRunBlob`。~~ **Phase 16 v2.20 废弃**：GlyphRunBlob 已移除；ElementTextData 改为 runtime-shape 字段集（§10 / §C.7）。
2. **ColorMatrix 20-float 特化**：见附录 D 的 `WriteValue<std::array<float, 20>>` 规范。
3. ~~**OutlineAll 模式比对策略**：独立基准图 folder~~ **v2.20 起 OutlineAll 路径从 API / CLI / 测试框架彻底移除**（见 §10.5 / §15.2 / §18.7）；当前仅保留 Render 分支。
...
### C.4 ImageAsset（Phase 16 后唯一保留的资源）

> **Phase 16 变更**：原 §C.4 定义了 `ImageAsset` + `FontAsset` + `FontAxis` + `FontSourceKind` + `GlyphRunKind` + `GlyphRunBlob`（含 `LayoutGlyph` / `ClassicGlyph` 嵌套）六个结构。v2.20 起仅保留 `ImageAsset`。字体相关结构全部废弃——字体信息直接嵌入 `ElementTextData`（§C.7），TextBlob 在 Inflater 端通过 `TextShaper` 运行时重建（§10）。
...
struct TextPayload {
  ...
  // Phase 16 v2.20: fontIndex 移除（随 PAGDocument::fonts[] 废弃），对齐 runtime-shape
  // 模式，字体信息直接携带 family/style 字符串
...
| `WriteGlyphRunBlob`/`ReadGlyphRunBlob` | — | Phase 16 v2.20 废弃（GlyphRunBlob 结构已移除，Baker/Codec 不再产出），见 §D.11 |
...
- ~~§D.6 `FontAssetTable.family` / `style` / `data`~~ （Phase 16 v2.20 随 FontAssetTable 废弃移除）
- §D.7 `Composition.id` / §D.8 `Layer.name`（maxBytes = `MAX_NAME_STRING_BYTES`）
- §D.11 `ElementText.text` / `fontFamily` / `fontStyle`（Phase 16 v2.20：ElementText body 新增三个 utf8string 字段，`text` 走 `MAX_TEXT_STRING_BYTES`；`fontFamily` / `fontStyle` 走 `MAX_NAME_STRING_BYTES`）
- ~~§D.11 `GlyphRunBlob` 内所有 `utf8string` 字段~~（Phase 16 v2.20 随 GlyphRunBlob 废弃移除）
...
- **强制用于**以下"安全关键索引字段"（值控制内存分配或循环上界）：
  ...
  - ~~`Layer.fontIndex` / `GlyphRunBlob.fontIndex`~~（§D.11 —— Phase 16 v2.20 随 FontAsset 废弃移除）
...
> **Decoder 强制上界**：读完 `varU32 assetCount` 后立即校验：ImageAssetTable 要求 `assetCount ≤ limits::MAX_IMAGES`。超限推 `StructureLimitExceeded=105` fatal 并 return（见 §H.1）。CompositionList 的 `compositionCount` 同理校验 `≤ MAX_COMPOSITIONS`（Baker 侧等价走 alias `BAKE_MAX_TOTAL_COMPOSITIONS`，P1-C v2.19）。（Phase 16 v2.20：原 `FontAssetTable.assetCount ≤ MAX_FONTS` 校验随 FontAssetTable 废弃而移除。）
...
**FontAssetTable**（Phase 16 v2.20 废弃）：

> **v2.20 Phase 16 起 Encoder 不写 `FontAssetTable=3` / `FontAsset=7`**。Decoder 若读到这两个 TagCode（例如老版本开发分支产物）走标准 `UnknownTagCode=400` warn + length skip 路径，不崩溃；**TagCode 数值保留不重用**（符合 §6.5 向前兼容）。
>
> 原字节布局保留在此供历史追溯 —— 老版本 Reader 仍需能按本布局解析旧产物（尽管 Phase 16 Encoder 不再生成）：

```
TagCode::FontAssetTable = 3                 [Phase 16 废弃，Encoder 不写]
body:
  varU32 assetCount
  repeat assetCount:
...
```
TagCode::FontAsset = 7                       [Phase 16 废弃，Encoder 不写]
body:
  ...
  dataLen bytes
  varU32      axisCount       # Decoder 强制 axisCount ≤ MAX_FONT_AXES=64
  repeat axisCount:
    u32       tag
    f32       defaultValue
    f32       minValue
...
| ElementText (50) | ElementTextData | **Phase 16 (v2.20)**: `Property<Point> position, Property<vector<Point>> anchors, utf8string text, utf8string fontFamily, utf8string fontStyle, f32 fontSize, u8 direction, u8 justification, f32 leading, f32 tracking, f32 firstBaseLine, f32 baselineShift, u8 textFlags, [if bit0] {Point boxTextPos, Point boxTextSize}, Color fillColor, Color strokeColor, f32 strokeWidth, Color backgroundColor, u8 backgroundAlpha`（字段顺序与 §C.7 ElementTextData 声明一致）|
...
**pathFlags / modifierFlags / textFlags 位域定义**（v2 统一常量）：
...
// Phase 16 v2.20 新增：ElementTextData 的布尔字段位压缩集合。
namespace pagx::pag::text_flags {
constexpr uint8_t BoxText        = 1u << 0;  // 1 = 带 boxTextPos/boxTextSize 段；0 = 两字段默认值，不出现在流中
constexpr uint8_t FauxBold       = 1u << 1;
constexpr uint8_t FauxItalic     = 1u << 2;
constexpr uint8_t ApplyFill      = 1u << 3;  // 默认 1（对齐 v1 TextDocument::applyFill=true）
constexpr uint8_t ApplyStroke    = 1u << 4;
constexpr uint8_t StrokeOverFill = 1u << 5;  // 默认 1（对齐 v1 TextDocument::strokeOverFill=true）
// bit 6-7 reserved
}
```
...
- `ElementText` 的 `boxTextPos / boxTextSize` 对 Point（每个 8 byte）仅当 `textFlags.BoxText=1` 时序列化；大多数独立 Text 是 non-box，可省 16 byte。
...
**GlyphRunBlob inline 布局**（Phase 16 v2.20 废弃）：

> v2.0-v2.19 原规划的 GlyphRunBlob 量化编码在 Phase 16 起**不再产出**。ElementText body 直接携带 v1 TextDocument 风格字段（上表 "ElementText (50)" 行）。Baker 不再写 GlyphRunBlob，Inflater 不再读；old format 的 `GlyphRun` 量化字节流不会再在任何 v2.20+ .pag 文件中出现。原量化编码节省 ~81% 的成果转嫁到 `TextShaper` runtime shape 的开销上——Inflater 端每个 Text element 额外花 O(N·字符数) 的 shape 时间；典型 200 字符段落 shape 耗时 < 1ms，相对 tgfx GPU 渲染总开销可忽略。
>
> Phase 16 后 ElementText body 结构简单直写即可（`Property<Point>` / `utf8string` / `Color` 等字段均为 ValueCodec 标准原语），不涉及动态位宽量化。
...
**风格强约束**：
- Write 函数：接受 `const ref`，返回 `void`，通过 `ctx->warn(...)` 推送告警（v2.20 P1-23 起统一 3 参签名）；
...
| convertText 371-385 + PrepareTextBlob 237-243 | `pagx::Text` | ElementTextData（Phase 16 v2.20：runtime-shape 模式，text/fontFamily/fontStyle/排版字段直接展平；原 GlyphRunBlob 原材料路径已废弃）|
...
enum class DiagnosticCode : uint16_t {
  ...
  FontSourceMissing         = 201,  // Phase 16 语义：pagx::Text 的 fontFamily 为空字符串，Baker 无法记录字体名（Inflater 将依赖 FontProvider fallback）
                                    //   v2.0-v2.19 原语义"内嵌/外部字体读取失败"已随 FontAsset 废弃而作废
  ...
  TextGlyphDataEmpty        = 206,  // 废弃（v2.20 起不再产出）。枚举值保留仅为 ABI append-only 约束；
                                    //   任何新代码严禁引用；旧引用点已在 Phase 16.2 TextBaker 重写时清除。
  EmptyDocument             = 207,  // 空 pagx（non-fatal informational）
  TextGlyphRunsDowngraded   = 208,  // Phase 16 新语义：pagx::Text.glyphRuns 非空（pre-shaped），Baker 丢弃 glyph
                                    //   ID + per-glyph xform，降级为 text + fontFamily 的 runtime-shape 路径
                                    //   v2.0-v2.19 的 GlyphRunKindInferred 同码，语义已随 GlyphRunBlob 废弃
  ...
  InflateFontCreateFailed   = 601,  // Phase 16 语义：FontProvider.getTypeface 返回 null 且 fallback 链也无可用 typeface；Text element 渲染为空白
                                    //   v2.0-v2.19 原语义"tgfx::Typeface 创建失败降级系统字体"已随 FontAsset 废弃
  InflateGlyphRunBuildFailed = 602, // Phase 16 语义：TextShaper::Shape 返回空 / 生成的 TextBlob 为 null；整个 ElementText 跳过
                                    //   v2.0-v2.19 原语义"GlyphRunRenderer 返回 null"已随 GlyphRunBlob 废弃
...
**消费侧 range-check 纪律**（P0-19 v2.20，强制）：部分错误码（`InvalidCompositionIndex=306`、`InflateMaskResolveFailed=603` 等）的 `contextIndex` 是 **Decoder 从字节流读出的原始值**，在推送 Diagnostic 时尚未与实际 `doc.compositions.size()` / `layerCount` 比对——即 `contextIndex` 可能 `≥ compositionCount`。消费方**必须**在解引用前按上下文做范围检查：
- `PAGLoader::Result` 侧：对照 `Result::compositionCount` 校验（§15.3）；
- `PAGExporter::Result` 侧：Exporter 路径不产出这类 "raw from bytes" 码，无需 range-check；
- 内部代码（Baker/Inflater）侧：contextIndex 由本地代码填写，源头已校验。
...
| FontSourceMissing (201) | TextBakerTest.cpp（Phase 16 v2.20 迁移：原 ResourceBakerTest 路径随 FontAsset 废弃；新触发点是 Baker 发现 pagx::Text.fontFamily 为空字符串） | TextBaker.FontFamilyEmpty |
...
constexpr size_t MAX_FONT_BYTES         = 50ull * 1024 * 1024;     // Phase 16 v2.20 起不再使用（FontAsset 废弃）；保留常量不动避免头文件 ABI 波动
constexpr uint32_t MAX_IMAGES           = 10000;
constexpr uint32_t MAX_FONTS            = 1000;                    // Phase 16 v2.20 起不再使用（FontAssetTable 废弃）；保留常量不动
constexpr uint32_t MAX_FONT_AXES        = 64;                      // Phase 16 v2.20 起不再使用（FontAxis 废弃）；保留常量不动
...
- Read 单个资源 size 超 MAX_IMAGE_BYTES → warn `ImageResourceSizeExceeded` + skip bytes + 跳过该资源（index 记为 UINT32_MAX）。（Phase 16 v2.20：`MAX_FONT_BYTES` / `FontResourceSizeExceeded` 随 FontAsset 废弃不再触发。）
...
- **MAX_IMAGE_BYTES**：ResourceBaker 读图片字节前检查；超限推 warning `ErrorCode::ImageResourceSizeExceeded`，该资源 index 记为 UINT32_MAX，引用点降级。（Phase 16 v2.20：`MAX_FONT_BYTES` / `ErrorCode::FontResourceSizeExceeded` 随 FontAsset 废弃不再触发；编号 408 保留不重用。）
...
### H.5 字体魔数校验（P0-17 v2.18；Phase 16 v2.20 废弃）

> **Phase 16 v2.20 废弃**：Baker 不再把字体 bytes 嵌入 .pag（改为 Inflater 端通过 `FontProvider` 从调用方 pre-register 的 typeface 注册表查询，见 §10 / §11.2）。`FontAsset::data` 结构体已移除，本节的 magic 白名单 no-op。
>
> PAGX 侧（`src/pagx/html/` / `src/pagx/svg/`）的字体嵌入路径仍可能在各自 Exporter 中复用类似 magic 白名单，但这些场景与 PAG v2 二进制格式无关——由各个子系统自行维护。
...
> 表格保留供 PAGX 自身 Exporter / FontEmbedder（非 PAG v2 路径）复用参考。
...
#### I.1.b tgfx-dependent 公共头（`PAGLoader.h` / `pag/FontProvider.h`）

`PAGLoader.h` 与 `pag/FontProvider.h`（Phase 16 v2.20 新增）是对外头中唯二允许依赖 tgfx 的，适用于"渲染消费方"用户。

**允许包含**：
- `pagx/Diagnostic.h`；
- `tgfx/layers/Layer.h` / `tgfx/core/Typeface.h`；
- STL 头。
...
**P0-20 v2.20 附加：varU32 解码安全预检**（Phase 1 开工前 30 分钟执行）：§D.1 要求 `readEncodedUint32` / `readEncodedUint64` 具备 **5/10 字节硬停 + 最短形式校验**两条安全约束。Phase 1 `ValueCodec.h` 是否需要落 `ReadEncodedUint32Safe` wrapper**取决于** v1 `EncodeStream` 是否已原生实现——开工前必须确认：

```bash
# 一次性读 v1 实现，确认 5/10 字节硬停与最短形式校验点位
grep -nE "readEncodedUint(32|64)\b" src/codec/utils/EncodeStream.cpp src/codec/utils/EncodeStream.h
# 关注：循环条件是否 `i < 5` 硬停；continuation bit 在第 5/10 字节时如何处理；
# 是否接受 0x80 0x00（非最短）还是 reject。
```
...
| v1 现状 | Phase 1 动作 |
|---|---|
| 已有 5/10 字节硬停 + 最短形式校验 | 直接 `using` v1 函数，不落 wrapper |
| 有硬停但无最短形式校验 | 落 `ReadEncodedUint32Safe` wrapper，仅补最短形式分支 |
| 两者都无 | 落完整 wrapper；`ValueCodecSafe.EncUint32.ContinuationAt5th` 测试（§18.4ter）覆盖硬停路径 |

预检未完成不得进入 Phase 1 exit gate。调查结论写进 Phase 1 PR 描述（"v1 现状 + 采纳方案"两行即可）。
...
- PAG v2 版本：0x02（本期；Phase 16 v2.20 ElementText schema 不兼容变更但 v2 未对外发布，不升 FORMAT_VERSION）；动画扩展不升版本号。

### 上次开工必读（开工前 AI / 人类 follow-up 的精简摘要）

v2.0 → v2.20 累计 9 轮专家评审（~85 个 P0/P1/P2 修复）+ 1 轮 Phase 15 实测暴露的架构回退。**开工前只需记下这些硬约束**，其他版本日志作"为什么这样设计"的补充阅读：
...
5. **资源零拷贝**：`ImageAsset::data` 是 `shared_ptr<const tgfx::Data>`（§11.1）；Inflater `MakeFromEncoded` 成功后立即 `data.reset()`；`PAGLoaderTest.ImageBytesReleasedAfterInflate` 断言 `use_count==1`。（Phase 16 v2.20 起 `FontAsset` 已废弃，原 `FontBytesReleasedAfterInflate` 断言对应删除。）
...
16. **Phase 16 v2.20 文本 runtime-shape 回退**（M5 里程碑）：Phase 15 实测发现 `tgfx::SystemFont::MakeFromName` 在 macOS/Linux/Android 硬 `return nullptr`（仅 Windows 实现 DirectWrite），Phase 8 "glyph ID 预存序列化" 架构不可行——所有文本 inflate 后 typeface null → TextBlob null → ElementText drop。**回退方向**：ElementTextData 存 text + fontFamily/fontStyle 字符串（对齐 v1 TextDocument），Inflater 调 v1 `TextShaper::Shape` 运行时 shape + 复用 v1 TextLayout 做段落布局。**接触面**：§10 / §11.2 / §C.4 / §C.7 / §D.6 / §D.11 / §G.2 (错误码 208 `TextGlyphRunsDowngraded` / 601/602 语义更新)；`FontAsset` / `GlyphRunBlob` / `FontAxis` / `FontSourceKind` / `PAGDocument::fonts[]` 全部废弃。**不升 FORMAT_VERSION**（v2 未对外发布）。**新增 `FontProvider` 接口**：调用方通过 `LayerInflater::Options::fontProvider` 注入自定义字体解析（默认 adapter 走全局 `pag::FontManager`）。详细设计见 [`pagx_to_pag_v2_phase16_text_redesign.md`](pagx_to_pag_v2_phase16_text_redesign.md)。
17. **v2.20 Review 收敛（P0+P1 文档修订）**：本次审核识别并落地——(a) **P0-18 Baker 输入 PAGX 生命周期约束**（§7.1，禁止并发修改/节点 GC，否则 `layerPathByPagxLayer` 指针 key 失效）；(b) **P0-19 Result::compositionCount**（§15.3，消费 `InvalidCompositionIndex=306` 等 "字节流原始值" contextIndex 前必须 range-check，见 §G.3）；(c) **P0-20 varU32 安全预检**（§I.5，Phase 1 开工前 30 分钟必做 v1 `readEncodedUint32` 调研，决定是否落 Safe wrapper）；(d) **P0-21 Fuzz corpus 规范**（§18.3bis + §18.3ter，CorruptBuilder 为唯一初始种子源，≥30 条覆盖 400/600 段错误码，corpus 上限 100 MB）；(e) **P1-22 覆盖率分模块目标**（§17 D3，Baker/Codec/Inflater 各 ≥80%、整体 ≥75%，取代旧 ≥85% 全局口径）；(f) **TagCode 240-299 "资源扩展段"预留删除**（§6.1，遵循"不超前设计"原则）；(g) **`FontMode::OutlineAll` 彻底移除**（§10.5 / §15.2 / §14.1 / §18.7，避免长期 SKIP 腐化）；(h) **P1-23 EncodeContext 独立化**（§8.5，替代裸指针聚合 `EncodeSession`，4 Context 形态对称）；(i) **P1-24 微型专项测试合并**（§18.4quater/quinta，PackLayerPath 3→1 参数化；ZeroCopyScope LeakDetection 双策略覆盖非 ASAN 环境）。
