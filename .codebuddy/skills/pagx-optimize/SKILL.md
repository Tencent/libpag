...
**Layer vs Group**: Layer creates an independent rendering surface (heavier, supports
styles/filters/mask). Group is a lightweight scope boundary (no extra surface, propagates
geometry to parent). **One Layer = one logical block**: each Layer should represent one
independent visual unit (a card, a button, a bar); within a block, use Groups for sub-elements.
...
> checklist and stacking order rules in `references/layer-vs-group.md` before applying.
> Key constraint: only downgrade Layers that are sub-elements of the **same** logical block;
> never merge distinct independent blocks into one Layer.
