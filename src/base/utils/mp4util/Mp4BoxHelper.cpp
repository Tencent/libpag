...
std::unique_ptr<ByteData> Mp4BoxHelper::ConcatMp4(VideoSequence* videoSequence) {
  size_t dataSize = videoSequence->mp4Header->length();
  int32_t mdatSize = 0;
  for (auto header : videoSequence->headers) {
    int32_t needSize = (int32_t)header->length();
    mdatSize += needSize;
  }
  for (auto frame : videoSequence->frames) {
    int32_t needSize = (int32_t)frame->fileBytes->length();
    mdatSize += needSize;
  }
  mdatSize += 8;
  dataSize += mdatSize;
...
void Mp4BoxHelper::WriteMdatBox(VideoSequence* videoSequence, SimpleArray* payload, int mdatSize) {
  ...
  int splitSize = 4;
  for (auto header : videoSequence->headers) {
    int32_t payLoadSize = (int32_t)header->length() - splitSize;
    payload->writeInt32(payLoadSize);
    payload->writeBytes(header->data(), payLoadSize, splitSize);
  }
  for (auto frame : videoSequence->frames) {
    int32_t payLoadSize = (int32_t)frame->fileBytes->length() - splitSize;
    payload->writeInt32(payLoadSize);
