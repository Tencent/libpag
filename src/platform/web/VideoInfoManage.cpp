...
VideoInfoManage::VideoInfoManage(std::shared_ptr<PAGFile> pagFile) : pagFile(pagFile) {
...
emscripten::val VideoInfoManage::getMp4DataById(const ID id) {
  ...
  if (iter != videoInfoMap.end()) {
    ...
    auto demuxer = std::make_unique<WebVideoSequenceDemuxer>(
        pagFile->getFile(), iter->second.videoSequence, pagFile.get());
...
float VideoInfoManage::getPlaybackRate() {
  float playbackRate = 1;
  if (pagFile != nullptr && pagFile->timeStretchModeInternal() == PAGTimeStretchMode::Scale) {
    playbackRate =
        pagFile->getFile()->duration() / ((pagFile->duration() / 1000000) * pagFile->frameRate());
  }
  return playbackRate;
}
