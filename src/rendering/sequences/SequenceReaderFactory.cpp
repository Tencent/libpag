...
std::shared_ptr<SequenceReader> SequenceReaderFactory::makeReader(PAGLayer* layer) const {
  if (sequence->composition->type() == CompositionType::Bitmap) {
    return std::make_shared<BitmapSequenceReader>(layer->getFile(),
  ...
  if (!VideoDecoder::HasExternalSoftwareDecoder()) {
    return std::make_shared<VideoSequenceReader>(layer, videoSequence);
  }
#endif
