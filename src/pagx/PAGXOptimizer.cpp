...
// ----------------------------------------------------------------------------
// Split long Path nodes at sub-path boundaries.
//
// Very long compound paths (typically Figma text exports that outline every
// glyph into a single `<path d="...">`) trigger the "Path with NNN verbs
// (> 1024), may cause slow rendering" warning in `pagx verify` and cost the
// renderer significantly more work than N smaller paths covering the same
// pixels. This pass replaces such a Path with several sibling Path elements
// whose combined geometry matches the original exactly:
//
//   1. Parse the source PathData into sub-paths (each `Move` starts one).
//   2. Cluster sub-paths so every "hole" (a sub-path whose control-point
//      bbox is contained within a previous sub-path in the same cluster)
//      stays together with its outer contour, which preserves fill-rule
//      behaviour for glyphs like O, B, g, 0.
//   3. Pack clusters into chunks whose total verb count stays under
//      `maxVerbsPerPath`.
//   4. Emit one new Path (and its own unnamed PathData) per chunk.
//
// When the source path collapses into one cluster (a single subpath, or a
// densely nested glyph that all bbox-contains into one outer), the splitter
// can optionally fall back to a chord split: for a single closed subpath it
// inserts an internal chord, producing two filled halves whose union covers
// the original area. This mirrors `--chord-split` in the matching Python
// reference at `tools/optimize_svg_paths.py`. It is opt-in (and therefore
// off by default) because the chord is visible if the path is stroked, and
// a Path element in PAGX cannot tell from itself whether a sibling Stroke
// will render it.
//
// Rendering is provably unchanged for the cluster-pack path; the chord-split
// fallback preserves the filled area exactly but adds one straight-edge that
// may be perceptible on stroked or partly transparent paths.
// ----------------------------------------------------------------------------

struct SubPathRange {
  size_t verbStart;   // inclusive
  size_t verbEnd;     // exclusive
  size_t pointStart;  // inclusive
  size_t pointEnd;    // exclusive
  float minX, minY, maxX, maxY;
  bool closed = false;  // true when the sub-path ends with a Close verb
  int verbCount() const {
    return static_cast<int>(verbEnd - verbStart);
  }
};

// Split the PathData into absolute sub-path ranges with control-point bboxes.
// The caller guarantees `data` is non-empty.
std::vector<SubPathRange> CollectSubPathRanges(const PathData* data) {
  std::vector<SubPathRange> subs;
  const auto& verbs = data->verbs();
  const auto& points = data->points();
  size_t pointIdx = 0;
  for (size_t vi = 0; vi < verbs.size(); vi++) {
    auto verb = verbs[vi];
    int numPts = PathData::PointsPerVerb(verb);
    if (verb == PathVerb::Move) {
      if (!subs.empty()) {
        subs.back().verbEnd = vi;
        subs.back().pointEnd = pointIdx;
      }
      SubPathRange range;
      range.verbStart = vi;
      range.pointStart = pointIdx;
      range.verbEnd = verbs.size();
      range.pointEnd = points.size();
      range.minX = range.maxX = points[pointIdx].x;
      range.minY = range.maxY = points[pointIdx].y;
      subs.push_back(range);
    } else if (numPts > 0) {
      for (int k = 0; k < numPts; k++) {
        const auto& p = points[pointIdx + static_cast<size_t>(k)];
        auto& cur = subs.back();
        cur.minX = std::min(cur.minX, p.x);
        cur.minY = std::min(cur.minY, p.y);
        cur.maxX = std::max(cur.maxX, p.x);
        cur.maxY = std::max(cur.maxY, p.y);
      }
    } else if (verb == PathVerb::Close && !subs.empty()) {
      subs.back().closed = true;
    }
    pointIdx += static_cast<size_t>(numPts);
  }
  if (!subs.empty()) {
    subs.back().verbEnd = verbs.size();
    subs.back().pointEnd = pointIdx;
  }
  return subs;
}

bool BBoxContains(const SubPathRange& outer, const SubPathRange& inner) {
  static constexpr float EPS = 1e-3f;
  return outer.minX - EPS <= inner.minX && outer.minY - EPS <= inner.minY &&
         outer.maxX + EPS >= inner.maxX && outer.maxY + EPS >= inner.maxY;
}

// Emit the verbs of sub-path `range` from `src` into `dst`. Absolute
// coordinates are preserved verbatim.
void AppendSubPath(PathData* dst, const PathData* src, const SubPathRange& range) {
  const auto& verbs = src->verbs();
  const auto& points = src->points();
  size_t pointIdx = range.pointStart;
  for (size_t vi = range.verbStart; vi < range.verbEnd; vi++) {
    auto verb = verbs[vi];
    switch (verb) {
      case PathVerb::Move:
        dst->moveTo(points[pointIdx].x, points[pointIdx].y);
        pointIdx += 1;
        break;
      case PathVerb::Line:
        dst->lineTo(points[pointIdx].x, points[pointIdx].y);
        pointIdx += 1;
        break;
      case PathVerb::Quad:
        dst->quadTo(points[pointIdx].x, points[pointIdx].y, points[pointIdx + 1].x,
                    points[pointIdx + 1].y);
        pointIdx += 2;
        break;
      case PathVerb::Cubic:
        dst->cubicTo(points[pointIdx].x, points[pointIdx].y, points[pointIdx + 1].x,
                     points[pointIdx + 1].y, points[pointIdx + 2].x, points[pointIdx + 2].y);
        pointIdx += 3;
        break;
      case PathVerb::Close:
        dst->close();
...
// Build the two halves of a chord-split for a single closed sub-path. Returns false when
// the sub-path is unsuitable (open, too short, or has no eligible mid-point verb). The two
// emitted halves together cover exactly the original filled area. The chord runs along the
// straight line between each half's start point and the split point; the chord is interior
// to the closed contour so it disappears under solid fills, but is potentially visible on
// stroked or alpha-composited renderings — which is why the caller has to opt in.
bool TryChordSplitSubPath(const PathData* src, const SubPathRange& sub, PathData* outFirst,
                          PathData* outSecond) {
  if (!sub.closed) return false;
  const auto& verbs = src->verbs();
  const auto& points = src->points();
  // Indices (within the sub-path's verb range) of segment-producing verbs that are eligible
  // mid-points: Line / Quad / Cubic. Move starts the sub-path and Close terminates it.
  std::vector<size_t> segIndices;
  for (size_t vi = sub.verbStart; vi < sub.verbEnd; vi++) {
    auto v = verbs[vi];
    if (v == PathVerb::Line || v == PathVerb::Quad || v == PathVerb::Cubic) {
      segIndices.push_back(vi);
    }
  }
  if (segIndices.size() < 4) return false;
  size_t mid = segIndices[segIndices.size() / 2];

  // Walk the sub-path once to recover the per-verb starting point index, then derive the
  // start point and the split point (= last on-curve point of verbs[mid]).
  std::vector<size_t> verbPointStart(sub.verbEnd - sub.verbStart, 0);
  size_t pointIdx = sub.pointStart;
  for (size_t vi = sub.verbStart; vi < sub.verbEnd; vi++) {
    verbPointStart[vi - sub.verbStart] = pointIdx;
    pointIdx += static_cast<size_t>(PathData::PointsPerVerb(verbs[vi]));
  }
  Point startPt = points[sub.pointStart];
  size_t midPoints = static_cast<size_t>(PathData::PointsPerVerb(verbs[mid]));
  Point splitPt = points[verbPointStart[mid - sub.verbStart] + midPoints - 1];

  outFirst->moveTo(startPt.x, startPt.y);
  for (size_t vi = sub.verbStart + 1; vi <= mid; vi++) {
    SubPathRange one{vi,
                     vi + 1,
                     verbPointStart[vi - sub.verbStart],
                     verbPointStart[vi - sub.verbStart] +
                         static_cast<size_t>(PathData::PointsPerVerb(verbs[vi])),
                     0,
                     0,
                     0,
                     0,
                     false};
    AppendSubPath(outFirst, src, one);
  }
  outFirst->lineTo(startPt.x, startPt.y);
  outFirst->close();

  outSecond->moveTo(splitPt.x, splitPt.y);
  for (size_t vi = mid + 1; vi < sub.verbEnd; vi++) {
    if (verbs[vi] == PathVerb::Close) break;
    SubPathRange one{vi,
                     vi + 1,
                     verbPointStart[vi - sub.verbStart],
                     verbPointStart[vi - sub.verbStart] +
                     ...
                     0,
                     false};
    AppendSubPath(outSecond, src, one);
  }
  outSecond->lineTo(splitPt.x, splitPt.y);
  outSecond->close();
  return true;
}

// Returns the set of replacement Path elements for `path`, or an empty
// vector when splitting isn't applicable (callers keep the original path in
// that case). This never mutates `path` or `path->data`.
std::vector<Element*> TrySplitLongPath(PAGXDocument* doc, Path* path, int verbThreshold,
                                       int maxVerbsPerPath, bool enableChordSplit) {
  if (path == nullptr || path->data == nullptr) return {};
  // Only split "plain" paths whose geometry is self-contained: layout constraints and
  // customData would semantically disagree on which child to attach to after splitting,
  // and `reversed=true` would produce a different draw order once the verb list is chopped.
  if (path->reversed) return {};
  if (ElementHasConstraints(path)) return {};
  if (!path->customData.empty()) return {};
  // A shared/named PathData resource may be referenced from other Paths; rewriting one
  // reference to point at fresh resources is still correct, but changing geometry of the
  // original is off-limits. Skipping the split keeps behaviour predictable.
  if (!path->data->id.empty()) return {};
  const auto& verbs = path->data->verbs();
  if (static_cast<int>(verbs.size()) <= verbThreshold) return {};
  if (path->data->isEmpty()) return {};

  auto subs = CollectSubPathRanges(path->data);
  if (subs.empty()) return {};

  // Cluster sub-paths: each "hole" (contained bbox) joins the most recent cluster.
  std::vector<std::vector<size_t>> clusters;
  clusters.emplace_back();
  clusters.back().push_back(0);
  for (size_t i = 1; i < subs.size(); i++) {
    bool attached = false;
    for (auto idx : clusters.back()) {
      if (BBoxContains(subs[idx], subs[i])) {
  ...
  // Greedy first-fit pack clusters into chunks under `maxVerbsPerPath`. A single cluster
  // exceeding the budget is placed in its own chunk (splitting inside a cluster would
  // break hole containment), so the final verb count may exceed the budget in this rare
  // case — the diagnostic still improves because the rest of the path is split off.
  std::vector<std::vector<size_t>> chunks;
  std::vector<size_t> current;
  int currentVerbs = 0;
  for (auto& cluster : clusters) {
    int clusterVerbs = 0;
    for (auto idx : cluster) clusterVerbs += subs[idx].verbCount();
    if (!current.empty() && currentVerbs + clusterVerbs > maxVerbsPerPath) {
      chunks.push_back(std::move(current));
      current.clear();
      currentVerbs = 0;
    }
    for (auto idx : cluster) current.push_back(idx);
    currentVerbs += clusterVerbs;
  }
  if (!current.empty()) chunks.push_back(std::move(current));

  // Per-chunk emission helper (lambda so chord-split can fall through to it).
  auto emit = [&](const std::vector<std::vector<size_t>>& finalChunks,
                  const std::vector<SubPathRange>& finalSubs,
                  const PathData* sourceData) -> std::vector<Element*> {
    std::vector<Element*> out;
    out.reserve(finalChunks.size());
    for (auto& chunk : finalChunks) {
      auto* newData = doc->makeNode<PathData>();
      for (auto idx : chunk) {
        AppendSubPath(newData, sourceData, finalSubs[idx]);
      }
      auto* newPath = doc->makeNode<Path>();
      newPath->data = newData;
      newPath->position = path->position;
      newPath->reversed = path->reversed;  // always false here, preserved for clarity
      newPath->sourceLine = path->sourceLine;
  ...
  if (chunks.size() >= 2) {
    return emit(chunks, subs, path->data);
  }

  // Single-chunk fallback: clustering collapsed to one (single sub-path, or a densely
  // nested glyph). Cluster splitting would break hole containment, so the only option
  // for shrinking the path further is the opt-in chord split, which is also limited
  // to the single-sub-path case to match the Python reference (`tools/optimize_svg_paths.py`).
  if (!enableChordSplit || subs.size() != 1) return {};
  auto* leftData = doc->makeNode<PathData>();
  auto* rightData = doc->makeNode<PathData>();
  if (!TryChordSplitSubPath(path->data, subs[0], leftData, rightData)) return {};
  if (static_cast<int>(leftData->verbs().size()) > maxVerbsPerPath ||
      static_cast<int>(rightData->verbs().size()) > maxVerbsPerPath) {
    return {};
  }
  std::vector<Element*> out;
  out.reserve(2);
  for (auto* data : {leftData, rightData}) {
    auto* newPath = doc->makeNode<Path>();
    newPath->data = data;
    newPath->position = path->position;
...
// Walks `elements` in place: any oversized Path is replaced by its sibling chunks at the
// same index, and any Group/TextBox is recursed into so its own children get the same
// treatment. Returns true when this list (or any descendant list) was modified, so the
// outer optimizer iteration knows to keep looping.
//
// History note: an earlier version rebuilt `elements` into a `result` vector but only
// began copying when a Path actually split, which silently dropped previous siblings if
// the first mutation came from a deep Group recursion. Mutating in place avoids that
// failure mode entirely (mirrors `_walk_and_split` in the Python reference).
bool SplitLongPathsInElements(PAGXDocument* doc, std::vector<Element*>& elements, int verbThreshold,
                              int maxVerbsPerPath, bool enableChordSplit) {
  bool changed = false;
  for (size_t i = 0; i < elements.size();) {
    ...
    if (type == NodeType::Group || type == NodeType::TextBox) {
      auto* group = static_cast<Group*>(el);
      changed |= SplitLongPathsInElements(doc, group->elements, verbThreshold, maxVerbsPerPath,
                                          enableChordSplit);
      i++;
      continue;
    }
    ...
    auto* path = static_cast<Path*>(el);
    auto splits = TrySplitLongPath(doc, path, verbThreshold, maxVerbsPerPath, enableChordSplit);
    if (splits.empty()) {
      i++;
      continue;
    }
    elements.erase(elements.begin() + static_cast<ptrdiff_t>(i));
    elements.insert(elements.begin() + static_cast<ptrdiff_t>(i), splits.begin(), splits.end());
    i += splits.size();
    changed = true;
  }
  return changed;
...
bool SplitLongPathsInLayer(PAGXDocument* doc, Layer* layer, int verbThreshold, int maxVerbsPerPath,
                           bool enableChordSplit) {
  bool changed = SplitLongPathsInElements(doc, layer->contents, verbThreshold, maxVerbsPerPath,
                                          enableChordSplit);
  for (auto* child : layer->children) {
    changed |= SplitLongPathsInLayer(doc, child, verbThreshold, maxVerbsPerPath, enableChordSplit);
  }
  return changed;
}
...
void OptimizeLayerList(PAGXDocument* doc, std::vector<Layer*>& layers,
  ...
  for (int iter = 0; iter < options.maxIterations; iter++) {
    ...
    if (options.splitLongPaths && options.maxVerbsPerPath > 0 &&
        options.maxVerbsPerPath < options.pathVerbThreshold) {
      for (auto* layer : layers) {
        changed |= SplitLongPathsInLayer(doc, layer, options.pathVerbThreshold,
                                         options.maxVerbsPerPath, options.enableChordSplit);
      }
    }
