...
static std::string WrapAsHTMLDocument(const std::string& fragment, float width, float height) {
  ...
  std::string bodyStyle = "margin:0;padding:0;width:" + w + "px;height:" + h +
                          "px;overflow:auto;font-family:sans-serif";
  std::string viewportStyle =
      "position:relative;width:" + w + "px;height:" + h + "px;overflow:visible";
  std::string result = "<!DOCTYPE html>\n<html>\n<head>\n<meta charset=\"utf-8\">\n";
  result += "<meta name=\"viewport\" content=\"width=device-width,initial-scale=1\">\n";
  // Keep the exported document at the composition's original size. Browsers expose the page
  // scrollbars when the viewport is smaller, while the HTML subset parser still sees absolute
  // body/canvas dimensions.
  result += "<style>\n";
  result += "html { margin:0;padding:0;min-width:" + w + "px;min-height:" + h + "px }\n";
