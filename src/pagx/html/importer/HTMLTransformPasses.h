 ...
 * render: with `S` the children's combined main-axis size and `R` the free space, the space-evenly
 * step is `g = R / (n + 1)`. Writing `space-between` while increasing the container's main-axis
 * padding by `g` on both ends places every child exactly where space-evenly would, because the
 * reduced free space `R - 2g` then divides into `n - 1` steps of `g`. The rewritten container is
 * also rendered identically by PAGX itself, so the substitution is lossless across engines.
 *
 * Conservative: bails out unless the container has a fixed px main-axis size, no in-flow child
 * declares `flex` grow, every in-flow child's main-axis size resolves to px, and the line has free
 * space left (overflowing lines are handled by SpaceJustifyOverflowCollapsePass, which runs
 * first). `flex-wrap` needs no gate: the subset filter drops the property and PAGX lays flex
 * containers out as a single line regardless, so the single-line equivalence is the one the
 * runtime applies. Emits `subset:space-evenly-padding-compensated` for each rewrite.
 */
