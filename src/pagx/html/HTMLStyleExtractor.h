...
class HTMLStyleExtractor {
 public:
   ...
   *
   * The emitted <style> block uses a multi-line layout: each selector on its
   * own line, each declaration indented two spaces on its own line. This keeps
   * the output diff-friendly and easy to read by humans.
   *
