...
export class PAGImage {
  ...
  public static fromPixels(
    ...
    const numBytes = pixels.byteLength * pixels.BYTES_PER_ELEMENT;
    const dataPtr = PAGImage.module._malloc(numBytes);
    const dataOnHeap = new Uint8Array(PAGImage.module.HEAPU8.buffer, dataPtr, numBytes);
  ...
  public static fromTexture(texture: PAGGlTexture, width: number, height: number, flipY: boolean) {
    return new PAGImage(PAGImage.module._PAGImage._FromTexture(texture.textureID, width, height, flipY));
  }
