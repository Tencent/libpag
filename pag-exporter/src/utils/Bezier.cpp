...
inline BezierPoint InterpolatePoint(const BezierPoint& v0, const BezierPoint& v1, const float& t) {
...
inline float Distance(const BezierPoint& a, const BezierPoint& b) {
  auto dx = a.x - b.x;
  auto dy = a.y - b.y;
  auto dz = a.z - b.z;
...
inline bool DistanceExceedsLimit(const BezierPoint& pt1, const BezierPoint& pt2, const float& precision) {
  return MaxFloat3(fabsf(pt2.x - pt1.x), fabsf(pt2.y - pt1.y), fabsf(pt2.z - pt1.z)) > precision;
}

inline bool CubicTooCurvy(const BezierPoint pts[4], const float& precision) {
  ...
  return DistanceExceedsLimit(pts[1], pt1, precision) || DistanceExceedsLimit(pts[2], pt2, precision);
}

static float ComputeCubicLength(const BezierPoint* points, float distance,
                                unsigned minT, unsigned maxT, const float& precision) {
  if (TSpanBigEnough(maxT - minT) && CubicTooCurvy(points, precision)) {
    ...
    distance = ComputeCubicLength(result, distance, minT, halfT, precision);
    distance = ComputeCubicLength(&result[3], distance, halfT, maxT, precision);
  } else {
...
float Bezier::GetCubicLength(float startX, float startY, float startZ,
  ...
  if (Point3DOnLine(start, end, control1, precision) && Point3DOnLine(start, end, control2, precision)) {
    return Distance(start, end);
  }
  BezierPoint points[4] = {start, control1, control2, end};
  return ComputeCubicLength(points, 0, 0, MaxBezierTValue, precision);
}
