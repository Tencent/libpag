...
| `scrollRect` | 见第 16 节 |
| `mask` | 见第 16 节 |
| `antiAlias="false"` | 无直接 CSS 等效，SVG 可用 `shape-rendering: crispEdges` |
...
控制背景是否穿透到子图层。CSS `backdrop-filter` 天然支持背景穿透。自动禁用条件（非 normal 混合模式、有滤镜、3D 变换）需在预处理中检测。
...
**P3 降级**：不支持时将 P3 转换为 sRGB 最近色值。P3 分量可能超出 [0,1]。
...
**带 matrix**：将 matrix 应用到 startPoint/endPoint。若 matrix 含非均匀缩放/旋转导致 CSS 无法精确表达，降级为 SVG `<linearGradient gradientTransform="...">`。
...
**带 matrix**：CSS `conic-gradient` 不支持渐变变换 → 降级为对元素整体应用 transform 或 SVG/Canvas。
...
降级方案（按优先级）：
1. SVG filter（feImage + 程序生成菱形渐变图像）
2. Canvas API 运行时生成纹理
3. 旋转 45° 径向渐变近似（视觉有差异）
...
| `repeat` + `clamp` | `repeat-x`（clamp 降级为 no-repeat） |
| `mirror` | 无原生支持 → SVG `<pattern>` 实现 4 格镜像 |
| `clamp` | CSS 无原生 clamp → 降级 `no-repeat` |
| `decal` | `no-repeat`（超出透明） |

**mirror 精确实现**：SVG `<pattern>` 包含 4 个 `<image>` 元素（原始 + X 翻转 + Y 翻转 + XY 翻转），pattern 宽高为图片的 2 倍。

**matrix**：CSS 通过 `background-size`/`background-position` 部分模拟。旋转/倾斜需 SVG `<pattern patternTransform="...">`。

**filterMode**：`nearest` → `image-rendering: pixelated`，`linear` → `image-rendering: auto`。
...
**文本填充**：逐字形填充。TextModifier 颜色覆盖公式：`finalColor = blend(original, override, override.alpha × |factor|)`。HTML 中对每个字形 `<span>` 设独立 `color`。
...
| `plusDarker` | **无 CSS 等效** → SVG `feBlend` 或 Canvas |

**plusDarker 降级**：`S + D - 1` 公式在 CSS 中无原生支持。需要 SVG `<feComposite>` 或 Canvas `globalCompositeOperation`。
...
→
```css
clip: rect(20px, 310px, 220px, 10px); /* 或用 clip-path */
overflow: hidden;
```

或更推荐：
```css
clip-path: inset({y}px {canvasW - x - w}px {canvasH - y - h}px {x}px);
```
...
|-----------|-----------|---------|
| DiamondGradient | ❌ | SVG filter / Canvas 生成纹理 |
...
| `dashAdaptive` | ❌ | 预计算缩放 dasharray |
| `plusDarker` blend mode | ❌ | SVG feBlend / Canvas |
| ImagePattern `mirror` | ❌ | SVG `<pattern>` 4 格镜像 |
| ImagePattern `clamp` | ❌ | 降级为 no-repeat |
| TextPath (完整语义) | ⚠️ 部分 | 预计算逐字形位置 |
| Repeater 小数 copies | ⚠️ 部分 | 最后副本 opacity 模拟 |
| `antiAlias="false"` | ⚠️ 部分 | `image-rendering: pixelated` 仅影响图片 |
| `excludeChildEffects` | ⚠️ 部分 | 大多数场景忽略 |
| `groupOpacity="false"` | ⚠️ 需拆分 | 将 opacity 分配到每个子渲染元素 |
| ConicGradient + matrix | ⚠️ 部分 | 对元素整体 transform 近似 |
