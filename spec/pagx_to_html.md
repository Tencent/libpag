...
除原生 HTML 外，还支持生成 React JSX 和 Vue 3 SFC 格式的组件代码。通过 CLI 参数或 API Options 控制输出格式。

### 19.1 架构设计：中间表示（IR）

采用完整 DOM 树 IR 架构。将现有 HTMLWriter 改为生成树状中间表示，再由不同 Serializer 序列化为各框架格式。

```
PAGXDocument → HTMLWriter → IR (HTMLNode 树)
                                    ├─ NativeSerializer → 原生 HTML
                                    ├─ ReactSerializer  → React JSX
                                    └─ VueSerializer    → Vue 3 SFC
```
...
```cpp
struct HTMLNode {
  enum class Type { Element, Text, RawSVG };

  Type type = Type::Element;
  std::string tag;                              // "div", "span", "svg", ...
  std::string className;                        // CSS class 名
  std::string id;                               // 元素 ID
  std::vector<std::pair<std::string, std::string>> attributes;  // data-*, etc.
  std::vector<std::pair<std::string, std::string>> styles;      // CSS 属性键值对
  std::vector<HTMLNode> children;
  std::string textContent;                      // Type::Text 的文本内容
  std::string rawContent;                       // Type::RawSVG 的原始 SVG 内容
};
```

**核心改动**：现有 HTMLWriter 中直接拼接字符串的逻辑改为构建 HTMLNode 树。`styles` 存储为键值对（如 `{"width", "100px"}`、`{"opacity", "0.8"}`），而非拼接好的 style 字符串。
...
class HTMLSerializer {
 public:
  virtual ~HTMLSerializer() = default;
  virtual std::string Serialize(const HTMLNode& root, const HTMLExportOptions& options) = 0;
};
```
...
保持当前行为，将 IR 序列化为标准 HTML：

```html
<div class="pagx-root" style="width:400px; height:300px; position:relative; overflow:hidden;">
  <svg style="position:absolute; width:0; height:0; overflow:hidden;">
    <defs><!-- SVG 定义 --></defs>
  </svg>
  <div class="pagx-layer" style="opacity:0.8; transform:translate(10px,20px);">
    ...
  </div>
</div>
```
...
**序列化规则**：
- `styles` 拼接为 `style="key:value; key:value;"`
- `class` 直接输出
- 自关闭标签（`<img>`, `<br>`）正确处理
...
```jsx
export default function PagxComponent() {
  return (
    <div className="pagx-root" style={{width: "400px", height: "300px", position: "relative", overflow: "hidden"}}>
      <svg style={{position: "absolute", width: 0, height: 0, overflow: "hidden"}}>
        <defs>{/* SVG 定义 */}</defs>
      </svg>
      <div className="pagx-layer" style={{opacity: 0.8, transform: "translate(10px,20px)"}}>
        ...
      </div>
    </div>
  );
...
|------|-----------|
| `class="..."` | `className="..."` |
| `style="k:v; k:v;"` | `style={{camelCase: "value"}}` |
| `for="..."` | `htmlFor="..."` |
| `tabindex="..."` | `tabIndex="..."` |
| `<!-- 注释 -->` | `{/* 注释 */}` |
| `stroke-width` | `strokeWidth`（SVG 属性保持连字符） |
| `clip-path` | `clipPath`（SVG 属性保持连字符） |

**CSS 属性名 camelCase 转换**：
...
**数值处理**：纯数值的 CSS 值（如 `opacity: 0.8`）输出为数字而非字符串；带单位的值（如 `width: "100px"`）保持字符串。

**SVG 元素内的属性**：SVG 标签内的属性（如 `<path>` 的 `stroke-width`、`fill-rule`、`clip-path`）在 React 中保持连字符形式，React 对 SVG 属性做了特殊处理，同时支持 camelCase 和原始形式。
...
<script setup>
// Generated by PAGX HTMLExporter
</script>
```
...
|------|----------|
| `class="..."` | `class="..."`（保持不变） |
| `style="k:v; k:v;"` | `:style="{camelCase: 'value'}"`（对象绑定） |
| `<!-- 注释 -->` | `<!-- 注释 -->`（保持不变） |

**style 对象绑定规则**：
- CSS 属性名转为 camelCase（同 React）
- 字符串值用单引号包裹：`{width: '100px'}`
- 数值类型直接输出：`{opacity: 0.8}`
- `-webkit-*` 前缀属性转为 `WebkitXxx`（Vue 支持）
...
struct HTMLExportOptions {
  ...
  std::string componentName = "PagxComponent";  // React/Vue 组件名
};
```
...
```
HTML options:
  --html-indent <n>               缩进空格数（默认 2，范围 0-16）
  --html-framework <framework>    输出框架：native / react / vue
  --html-component-name <name>    组件名称（React/Vue，默认 PagxComponent）
```

**格式推断规则**（按优先级）：

1. 显式指定 `--html-framework` → 直接使用
2. 根据 `--output` 文件扩展名推断：`.jsx` → React，`.vue` → Vue，`.html` → Native
3. 两者都未指定且无法推断 → 报错：
   ```
   pagx export: error: cannot infer HTML framework, use --html-framework (native/react/vue)
   or specify an output file with .html/.jsx/.vue extension
   ```
...
```bash
# 原生 HTML
pagx export --input icon.pagx --output icon.html
pagx export --input icon.pagx --format html

# React JSX
pagx export --input icon.pagx --output Icon.jsx
pagx export --input icon.pagx --format html --html-framework react
pagx export --input icon.pagx --format html --html-framework react --html-component-name IconButton

# Vue 3 SFC
pagx export --input icon.pagx --output Icon.vue
pagx export --input icon.pagx --format html --html-framework vue --html-component-name IconButton
```
...
新增 `src/pagx/html/HTMLNode.h`，定义 `HTMLNode` 结构体和基本操作。

#### 第二步：重构 HTMLWriter

将现有 `HTMLBuilder`（字符串拼接器）改为 `HTMLNodeBuilder`（树构建器）。核心改动：

- `openTag("div")` → `pushNode(HTMLNode{.tag = "div"})`
- `addAttr("class", "pagx-layer")` → `currentNode().className = "pagx-layer"`
- `addStyle("width", "100px")` → `currentNode().styles.push_back({"width", "100px"})`
- `closeTag()` → `popNode()`（将当前节点添加为父节点的子节点）

HTMLWriter 的业务逻辑（图层遍历、几何转换、颜色计算等）不变，只改输出端。

#### 第三步：实现三个 Serializer

- `NativeHTMLSerializer`：从 IR 生成原生 HTML（复现当前行为）
- `ReactJSXSerializer`：从 IR 生成 React JSX
- `VueSFCSerializer`：从 IR 生成 Vue 3 SFC

#### 第四步：更新 API 和 CLI

- 扩展 `HTMLExportOptions`，增加 `framework` 和 `componentName`
- 修改 `HTMLExporter::ToHTML()` 内部流程：生成 IR → 根据 framework 选择 Serializer → 输出
- 修改 `CommandExport.cpp`，增加 HTML 导出支持和框架参数

#### 第五步：测试

在 `PAGXHtmlTest.cpp` 中增加 React 和 Vue 输出测试，验证：
- 属性映射正确性（class → className、style 对象化）
- 文件结构正确性（JSX 导出函数、Vue template/script 标签）
- SVG defs 内联正确性
- 特殊字符转义（JSX 中的 `{}`、Vue 模板中的 `{{}}`）

### 19.6 代码组织

```
include/pagx/
  HTMLExporter.h               （修改：扩展 Options + HTMLFramework 枚举）
src/pagx/html/
  HTMLNode.h                   （新增：IR 节点定义）
  HTMLNodeBuilder.h            （新增：树构建器，替代现有 HTMLBuilder）
  HTMLSerializer.h             （新增：Serializer 接口）
  NativeHTMLSerializer.cpp     （新增：原生 HTML 序列化）
  ReactJSXSerializer.cpp       （新增：React JSX 序列化）
  VueSFCSerializer.cpp         （新增：Vue SFC 序列化）
  HTMLExporter.cpp             （修改：改用 HTMLNodeBuilder + Serializer）
src/cli/
  CommandExport.cpp            （修改：增加 HTML 导出 + 框架参数）
test/src/
  PAGXHtmlTest.cpp             （修改：增加 React/Vue 测试）
```
