...
| `endPoint` | Point | `1,1` | 终点 |
| `matrix` | Matrix | 单位矩阵 | 变换矩阵 |
| `fitsToGeometry` | boolean | true | `startPoint`/`endPoint` 是否按几何元素包围盒的归一化 0-1 坐标解释，详见[颜色源坐标系统](#颜色源坐标系统) |
...
| `matrix` | Matrix | 单位矩阵 | 变换矩阵 |
| `fitsToGeometry` | boolean | true | `center`/`radius` 是否按几何元素包围盒的归一化 0-1 坐标解释，详见[颜色源坐标系统](#颜色源坐标系统) |
...
| `matrix` | Matrix | 单位矩阵 | 变换矩阵 |
| `fitsToGeometry` | boolean | true | `center` 是否按几何元素包围盒的归一化 0-1 坐标解释，详见[颜色源坐标系统](#颜色源坐标系统) |
...
| `matrix` | Matrix | 单位矩阵 | 作用于图片本地坐标系（原始图片 rect，原点在左上）的变换矩阵 |
| `scaleMode` | ScaleMode | letterBox | 将变换后的图片适配到每个几何元素包围盒时使用的缩放规则。为 `none` 时图片直接放置在图层坐标系中、不做适配，详见[颜色源坐标系统](#颜色源坐标系统) |
...
除纯色外，所有颜色源（渐变、图片填充）都同时支持相对几何元素包围盒的归一化坐标空间和几何元素本地坐标空间两种解释方式，分别由不同的属性控制：

- **渐变**：由 `fitsToGeometry` 属性控制（默认 `true`）。
- **ImagePattern**：由 `scaleMode` 属性控制。只要 `scaleMode` 不为 `none`，图片就会适配到每个几何元素的包围盒；`scaleMode="none"` 时图片直接放置在图层坐标系中。

**归一化包围盒空间（默认）**：

渐变参数（`startPoint`、`endPoint`、`center`、`radius` 等）按几何元素包围盒的 `(0, 0)`-`(1, 1)` 归一化坐标解释；ImagePattern 图片按 `scaleMode` 规则适配每个几何元素的包围盒。此模式下颜色源**会自动适配所使用的每个几何元素的尺寸**，因此多个不同尺寸的形状共用同一渐变或图案时，每个形状都能获得正确适配的填充。

**本地坐标空间（可选）**：

在渐变上设置 `fitsToGeometry="false"`、在 ImagePattern 上设置 `scaleMode="none"`，可让参数按几何元素的本地坐标系解释。此模式下颜色源为绝对定位；多个几何元素共用同一颜色源时会共享一份连续填充。`matrix` 属性在所选空间之上再叠加应用。

**变换行为**：

1. **外部变换会同时作用于几何和颜色源**：Group 的变换、Layer 的矩阵等外部变换会整体作用于几何元素及其颜色源，两者一起缩放、旋转、平移。

2. **修改几何属性会影响归一化填充，但不会影响绝对填充**：归一化模式下填充会重新适配到几何新的包围盒，因此改 Rectangle 的 `width`/`height` 时渐变或图案会自动放缩覆盖新的形状。在本地坐标模式（`fitsToGeometry="false"` / `scaleMode="none"`）下，颜色源坐标不会跟随几何属性变化。

**示例**：在本地坐标空间中绘制一条对角线方向的线性渐变：

> [Sample](samples/color_source_coordinates.pagx)

- 设置 `fitsToGeometry="false"` 且在 300×300 区域内：对该图层应用 `scale(2, 2)` 变换，矩形变为 600×600，渐变也随之放大，视觉效果保持一致
- 设置 `fitsToGeometry="false"` 且直接把 Rectangle 的 `width`/`height` 改为 600,600：矩形变为 600×600，但渐变坐标不变，只覆盖左上四分之一
- 使用默认 `fitsToGeometry="true"`：无论 Rectangle 的尺寸如何变化，渐变都会自动适配整个形状的包围盒
...
几何元素提供可渲染的形状。所有几何元素以及 TextPath、TextBox 和 Group 都支持约束属性用于容器内定位——完整属性列表、定义和行为见 §4.3。各元素属性表中不再重复列出约束属性。
