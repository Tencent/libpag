...
| `left` | float | - | 作为子 Layer 时，距父容器左边的距离（见 §4.3） |
| `right` | float | - | 作为子 Layer 时，距父容器右边的距离（见 §4.3） |
| `top` | float | - | 作为子 Layer 时，距父容器上边的距离（见 §4.3） |
| `bottom` | float | - | 作为子 Layer 时，距父容器下边的距离（见 §4.3） |
| `centerX` | float | - | 作为子 Layer 时，中心相对于父容器水平中心的偏移（见 §4.3） |
| `centerY` | float | - | 作为子 Layer 时，中心相对于父容器垂直中心的偏移（见 §4.3） |
...
| `left` | float | - | 约束定位：元素左边缘到容器左边缘的距离（见 §4.3） |
| `right` | float | - | 约束定位：元素右边缘到容器右边缘的距离（见 §4.3） |
| `top` | float | - | 约束定位：元素上边缘到容器上边缘的距离（见 §4.3） |
| `bottom` | float | - | 约束定位：元素下边缘到容器下边缘的距离（见 §4.3） |
| `centerX` | float | - | 约束定位：元素中心相对于容器水平中心的偏移（见 §4.3） |
| `centerY` | float | - | 约束定位：元素中心相对于容器垂直中心的偏移（见 §4.3） |
...
| `left` | float | - | 约束定位：距容器左边缘的距离（见 §4.3） |
| `right` | float | - | 约束定位：距容器右边缘的距离（见 §4.3） |
| `top` | float | - | 约束定位：距容器上边缘的距离（见 §4.3） |
| `bottom` | float | - | 约束定位：距容器下边缘的距离（见 §4.3） |
| `centerX` | float | - | 约束居中：相对容器水平中心的偏移（见 §4.3） |
| `centerY` | float | - | 约束居中：相对容器垂直中心的偏移（见 §4.3） |
