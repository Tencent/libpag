...
| 属性 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `position` | Point | 0,0 | 中心点 |
| `radius` | float | (必填) | 渐变半径 |
| `matrix` | Matrix | 单位矩阵 | 变换矩阵 |

**计算**：对于点 P，其颜色由 `distance(P, position) / radius` 决定。
...
**计算**：对于点 P，其颜色由 `atan2(P.y - position.y, P.x - position.x)` 在 `[startAngle, endAngle]` 范围内的比例决定。
...
**计算**：对于点 P，其颜色由切比雪夫距离 `max(|P.x - position.x|, |P.y - position.y|) / radius` 决定。
