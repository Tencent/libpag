...
同时设置 `left` + `right`（或 `top` + `bottom`）定义一个目标区域（容器尺寸减去两侧间距）。类似地，百分比尺寸（如 `width="50%"`）从容器计算尺寸。不同节点类型的响应方式：
...
| Polystar、Path、Text、TextPath | 等比缩放适配 | 单轴设置时紧贴该轴边界等比缩放；双轴设置时取两轴中较小的缩放因子（fit 模式），在较长轴方向居中 |

百分比尺寸在单轴上产生目标尺寸，无需 `left`/`right`/`top`/`bottom`。目标尺寸按 `target = 父容器内缩尺寸 × N / 100` 计算，其中「父容器内缩尺寸」为父容器该轴布局尺寸减去 `padding`。各元素的响应方式与对边约束表对应，但驱动源是百分比目标而非两侧间距：

| 元素 | `width="N%"` / `height="N%"` 行为 | 说明 |
|------|----------------------------------|------|
| Rectangle、Ellipse | 拉伸形状 | 在该轴上 `size.width = 父容器内缩宽 × N / 100`，改变渲染形状 |
| TextBox | 拉伸排版区域 | `width`/`height` 设为百分比目标尺寸，改变文字排版范围 |
| Group、Layer | 推导布局尺寸 | 在该轴上将布局宽/高设为百分比目标尺寸，内部子元素按新尺寸重新布局 |
| Polystar、Path、Text、TextPath | 等比缩放适配 | 将百分比目标尺寸作为该轴的目标值等比缩放（公式与单轴对边约束相同；若双轴均设百分比，取两轴中较小的缩放因子——fit 模式） |
