...
| `matrix` | Matrix | 单位矩阵 | 在所选坐标空间中叠加的变换矩阵，详见[颜色源坐标系统](#color-source-coordinate-system) |
| `fitsToGeometry` | boolean | true | 渐变的坐标空间。`true` 时为几何元素自身包围盒的归一化 0-1 坐标系；`false` 时为父级容器（Layer 或 Group）的坐标系。详见[颜色源坐标系统](#color-source-coordinate-system) |
...
| `matrix` | Matrix | 单位矩阵 | 作用于图片本地坐标系的变换矩阵，详见[颜色源坐标系统](#color-source-coordinate-system) |
| `scaleMode` | ScaleMode | letterBox | 将变换后的图片适配到每个几何元素包围盒时使用的缩放规则。为 `none` 时图片直接放置在父级容器（Layer 或 Group）的坐标系中、不做适配，详见[颜色源坐标系统](#color-source-coordinate-system) |
...
**路径起点**：矩形路径从**右上角**开始，顺时针方向绘制（`reversed="false"` 时）。线段退化情况下，路径从**左上角**开始延伸到右下角（`reversed="true"` 时方向相反）。
