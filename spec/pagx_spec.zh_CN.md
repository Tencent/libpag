...
TextBox 继承 Group 的所有属性（`position`、`anchor`、`rotation`、`scale`、`skew`、`skewAxis`、`alpha`）和约束属性。`position` 属性指定文本区域左上角在父坐标系中的坐标。推荐使用约束属性（`left`/`top`）进行定位——设置约束属性时，`position` 由约束系统自动计算。
