...
- **构建指令**：定义构建时 SVG 导入机制（内联 `<svg>` 和 `import` 属性）
...
TextBox 继承 Group 的所有属性（`position`、`anchor`、`rotation`、`scale`、`skew`、`skewAxis`、`alpha`、`padding`）和约束属性。`padding` 属性内缩文本排版区域和非 Text 子元素的约束参考系。`position` 属性指定文本区域左上角在父坐标系中的坐标。推荐使用约束属性（`left`/`top`）进行定位——设置约束属性时，`position` 由约束系统自动计算。
...
`pagx resolve` 命令处理 PAGX 文件中所有 SVG 导入指令：

1. 对于每个包含内联 `<svg>` 子节点或 `import` 属性的 Layer，读取 SVG 内容（内联元素或外部文件）
...
4. 从源文件尺寸设置 Layer 的 `width` 和 `height`（如 SVG 的 `viewBox` 或
   `width`/`height` 属性）
...
处理 PAGX 文件的工具对未解析的 SVG 导入指令的处理方式：

- **`pagx verify`**：自动解析所有 SVG 导入后再进行检查。如果解析失败，报告错误。
- **`pagx render`**：报告错误 —— `unresolved SVG import`，拒绝渲染。
...
| **构建指令** | （内联 `<svg>`、`import` 属性） | 构建时 SVG 导入指令。Layer 的内联 `<svg>` 子元素和 `import`/`importFormat` 属性通过 `pagx resolve` 解析为原生 PAGX 节点。 |
