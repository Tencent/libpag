...
| `<ViewModel>` | viewModel | ViewModel 模式，声明此 Composition 内可用于数据绑定的属性 |
...
| `timelines` | Timeline[] | [] | 当此图层引用 Composition 时激活的 Timeline 条目 |
...
`<Animation>` 定义一个命名的时间线。它在 `<Animations>` 容器中声明（放在 Document 或 Composition 内部均可），通过 `id` 标识，且 id 在所属作用域内必须唯一。动画由 Layer 的 `<Timelines>` 条目通过 `<Animation ref="@id"/>` 引用。

| 属性 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `id` | string | (必填) | 唯一标识符，供 Layer 的 `<Timelines>` 引用 |
| `duration` | Frame | (必填) | 动画总帧数 |
...
`type` 属性指定通道值类型，支持以下取值（均为小写）：

| `type` 值 | 说明 |
|-----------|------|
| `float` | 数值（透明度、缩放、旋转等），也可写 `number` |
| `bool` | 布尔值（开/关） |
| `int` | 整数值，也可写 `enum` |
| `string` | 文本值，也可写 `text` |
| `image` | 图片资源引用，也可写 `imageRef` |
| `color` | 颜色值（RGBA） |
| `matrix` | 2D 变换矩阵 |
...
| `Bezier` | 使用 `bezier-out` 和下一个关键帧的 `bezier-in` 控制柄进行三次贝塞尔缓动 |
...
| `bezier-in` | string | — | 三次贝塞尔缓入控制点，格式为 `"x,y"`，仅 `Bezier` 模式生效 |
| `bezier-out` | string | — | 三次贝塞尔缓出控制点，格式为 `"x,y"`，仅 `Bezier` 模式生效 |
...
`<StateMachine>` 定义状态图。它在 `<Animations>` 容器中声明并通过 `id` 标识，由 Layer 的 `<Timelines>` 通过 `<StateMachine ref="@id"/>` 引用。它声明 **输入**（宿主代码用来推送值的命名端口）和 **区域**（并行状态图）。每个区域拥有自己的状态和转换集合，独立推进 — 这借鉴了 UML 状态机的正交区域概念，可在建模独立行为维度时避免组合状态爆炸。
...
| `type` | StateMachineInputType | `bool` | 输入值类型 |
...
| 属性 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `name` | string | (必填) | 区域名称，在所属 StateMachine 内唯一 |
...
| `bezier-out` | string | — | 贝塞尔缓出控制柄，格式 `"x,y"`（仅 `interpolation="bezier"` 时生效） |
| `bezier-in` | string | — | 贝塞尔缓入控制柄，格式 `"x,y"`（仅 `interpolation="bezier"` 时生效） |
...
被引用的 `<StateMachine>` 定义在 `<Animations>` 容器中；其结构见 §8.2。
...
`<ViewModel>` 定义 ViewModel 模式。通过 `id` 标识，可被 Composition 或其他 ViewModel（类型为 `viewModel` 的 `<Property>`）引用。每个 `<Composition>` 或文档最多放置一个 `<ViewModel>`。DataBind 与 DataConverter 则可在同一容器下放置多个。
...
| `type` | ViewModelPropertyType | number | 属性的值类型 |

**ViewModelPropertyType** 取值（大小写不敏感，`Number` 与 `number` 均可）：
...
| 枚举 | 值 |
|------|------|
| **ViewModelPropertyType** | `number`, `string`, `boolean`, `color`, `image`, `viewModel`, `enum`, `trigger`（大小写不敏感） |
