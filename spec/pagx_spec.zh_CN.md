...
| `includeInLayout` | bool | true | 是否参与父容器的布局排列（见 §6） |
| `left` | float | - | 作为子 Layer 时，距父容器左边的距离（见 §6.3） |
| `right` | float | - | 作为子 Layer 时，距父容器右边的距离（见 §6.3） |
| `top` | float | - | 作为子 Layer 时，距父容器上边的距离（见 §6.3） |
| `bottom` | float | - | 作为子 Layer 时，距父容器下边的距离（见 §6.3） |
| `centerX` | float | - | 作为子 Layer 时，中心相对于父容器水平中心的偏移（见 §6.3） |
| `centerY` | float | - | 作为子 Layer 时，中心相对于父容器垂直中心的偏移（见 §6.3） |
...
| 属性 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `position` | Point | (bounding box 中心) | 中心点坐标。未设置时默认为 `(size.width/2, size.height/2)`，使左上角对齐原点 |
...
| 属性 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `position` | Point | (bounding box 中心) | 中心点坐标。未设置时默认为 bounding box 的中心，使左上角对齐原点 |
...
| `left` | float | - | 约束定位：距容器左边缘的距离（见 §6.3） |
| `right` | float | - | 约束定位：距容器右边缘的距离（见 §6.3） |
| `top` | float | - | 约束定位：距容器上边缘的距离（见 §6.3） |
| `bottom` | float | - | 约束定位：距容器下边缘的距离（见 §6.3） |
| `centerX` | float | - | 约束居中：相对容器水平中心的偏移（见 §6.3） |
| `centerY` | float | - | 约束居中：相对容器垂直中心的偏移（见 §6.3） |
...
自动布局让元素声明布局意图，由引擎自动计算坐标和尺寸。布局分两层：**容器布局**控制 Layer 之间的排列，**约束布局**控制 Layer 内部元素的定位。两者可以单独使用，也可以组合使用。所有布局计算在解析阶段一次性完成，之后布局属性即被丢弃，运行时只保留计算结果。

### 6.1 布局尺寸（Layout Size）

`width`/`height` 属性声明容器的布局尺寸，是两种布局机制的共同基础——容器布局用它确定子 Layer 的可用空间，约束布局用它作为定位的参考系。Layer 和 Group 都可以拥有布局尺寸。
...
布局尺寸本身不产生直接的渲染效果，仅向下传递尺寸信息供子元素的约束布局使用。布局尺寸和约束计算在元素的本地坐标系中完成，变换（`matrix` 属性）在布局计算之后作用，不参与也不影响布局。
...
| `center` | 交叉轴居中对齐 |
| `end` | 交叉轴末尾端对齐 |
| `stretch` | 拉伸填满交叉轴可用空间（仅对未设交叉轴尺寸的子 Layer 生效） |
...
设了 `layout` 的 Layer，默认所有子 Layer 的位置由布局引擎决定。设置 `includeInLayout="false"` 的子 Layer 脱离布局流——不参与排列、不占用空间、不影响容器测量，但仍然可见，可使用 `x`/`y` 或约束属性定位。适用于在布局容器内放置装饰层、背景层或角标等不应参与排列的元素。脱离布局流的子 Layer 始终可以使用约束属性（`left`/`right`/`top`/`bottom`/`centerX`/`centerY`），不受父容器 `layout` 模式的限制。

```xml
<Layer width="400" height="300" layout="vertical" gap="12">
  <Layer><!-- 参与布局 --></Layer>
  <Layer><!-- 参与布局 --></Layer>
  <!-- 角标：脱离布局流，使用 x/y 绝对定位 -->
...
**生效条件**：约束属性总是参照元素的**直接父容器**（Layer 或 Group）的布局尺寸，逐层向下传递——每层容器先确定自身尺寸，其子元素的约束再以该尺寸为参考系。由于引擎会自动测量容器尺寸（见 §6.1），约束通常都能生效。不同约束对容器尺寸的依赖程度不同：`left`/`top` 单独使用时，定位公式不涉及容器尺寸（如 `tx = left - bounds.x`），在任何情况下都能正确工作；`right`/`bottom`/`centerX`/`centerY` 以及对边约束需要引用容器尺寸来计算位置。
...
所有图层内容节点拥有 `position` 属性，表示元素锚点在父坐标系中的绝对坐标。约束布局的计算结果写入 `position`。各元素的锚点位置不同：
