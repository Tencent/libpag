...
**图层内容**是指图层的下层内容、子图层和上层内容的完整渲染结果（渲染流程中的步骤 2-5），不包含图层样式和图层滤镜。图层样式基于图层内容计算效果。例如，当填充为下层、描边为上层时，描边会绘制在子图层之上，但投影阴影仍然基于包含填充、子图层和描边的完整图层内容计算。

#### 图层轮廓（Layer Contour）

**图层轮廓**从图层中所有有绘制器参与的几何区域提取不透明覆盖范围，生成一个二值（不透明或完全透明）遮罩。没有绘制器的几何元素不参与。按颜色源类型的转换规则：

- **纯色填充和渐变填充**：原始颜色替换为不透明白色
- **图片填充**：完全透明的像素保留透明；其余像素均转为完全不透明
- **alpha=0 的填充**：透明度完全为 0 的填充所绘制的几何同样参与（视为不透明覆盖）
...
**excludeChildEffectsInLayerStyle**：当值为 `false`（默认）时，图层样式（如 DropShadowStyle）基于完整的图层内容计算，包含子图层的渲染结果。当值为 `true` 时，图层样式仅基于当前图层自身的绘制内容（Fill/Stroke）计算，不包含子图层的渲染结果。
...
2. 根据 `shape` 计算每个字形的原始影响值（0~1）
3. 多个选择器按 `mode` 组合

**变换应用**：

最终 `factor` 由原始影响值乘以 `weight` 后钳制到 [-1, 1] 得出：
