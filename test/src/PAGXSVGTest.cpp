...
// Regression for the mask-def cache key. A single mask Layer referenced by two owners under
// different MaskType values must produce two distinct <mask> defs with the correct mask-type
// attribute on each; sharing one cache slot across MaskTypes silently routed the second owner
// to the wrong mask channel (alpha vs luminance).
...
// Regression for the M3 fix. A mask Layer referenced by multiple owners under the same MaskType
// must produce exactly one <mask> def — the cache reuses the existing def id rather than
// re-emitting a duplicate-id element into <defs>.
...
// Regression for the M3 fix at the Contour role. A mask Layer used as both Alpha mask and
// Contour clipPath must produce one <mask> def AND one <clipPath> def; sharing the cache across
// roles would have suppressed the second def or routed the contour owner to the wrong element
// type.
