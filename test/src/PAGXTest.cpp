 ...
 * overlap the wrapped lines. Loads a PAGX file with a vertical layout containing a TextBox (using
 * left+right constraints for width) followed by a colored rectangle. If wrapping height is correct,
 * the rectangle appears below all text lines; if not, it covers the second line.
 */
PAGX_TEST(PAGXTest, TextBoxConstraintWidthWrapRender) {
  auto pagxPath = ProjectPath::Absolute("resources/apitest/textbox_constraint_wrap.pagx");
  auto doc = pagx::PAGXImporter::FromFile(pagxPath);
  ASSERT_TRUE(doc != nullptr);
  ASSERT_TRUE(doc->errors.empty());
...
/**
 * TextBox with explicit height but no width, constraint provides width. The explicit height should
 * be preserved — setLayoutSize should not overwrite it with re-typeset results.
 */
PAGX_TEST(PAGXTest, TextBoxExplicitHeightPreserved) {
  auto doc = pagx::PAGXDocument::Make(400, 400);
  auto layer = doc->makeNode<pagx::Layer>();
  doc->layers.push_back(layer);
  layer->width = 400;
  ...
  auto text = doc->makeNode<pagx::Text>();
  text->text = "ABCDEFGHIJ KLMNOPQRST UVWXYZ abcdefghij klmnopqrst uvwxyz";
  text->fontFamily = typeface->fontFamily();
  text->fontStyle = typeface->fontStyle();
  text->fontSize = 24;
  textBox->elements.push_back(text);
  textBox->elements.push_back(doc->makeNode<pagx::Fill>());
  layer->contents.push_back(textBox);
  ...
  EXPECT_FLOAT_EQ(textBox->layoutWidth, 360.0f);
  EXPECT_FLOAT_EQ(textBox->layoutHeight, 55.0f);
}

/**
 * Nested Layer containing TextBox with left+right constraints in a vertical flex layout.
 * The intermediate Layer should grow its height to fit the wrapped text, and the sibling
 * Layer should be positioned after the grown height.
 */
PAGX_TEST(PAGXTest, TextBoxNestedLayerHeightPropagation) {
  ...
  // Intermediate Layer (content-measured, no explicit w/h) containing TextBox.
  auto middleLayer = doc->makeNode<pagx::Layer>();
  auto textBox = doc->makeNode<pagx::TextBox>();
  textBox->left = 0;
  textBox->right = 0;
  auto text = doc->makeNode<pagx::Text>();
  text->text = "AAAA BBBB CCCC DDDD EEEE FFFF GGGG";
  text->fontFamily = typeface->fontFamily();
  text->fontStyle = typeface->fontStyle();
  text->fontSize = 16;
  textBox->elements.push_back(text);
  textBox->elements.push_back(doc->makeNode<pagx::Fill>());
  middleLayer->contents.push_back(textBox);

  // Sibling Layer with fixed height.
  auto sibling = doc->makeNode<pagx::Layer>();
  sibling->height = 30;

  parent->children = {middleLayer, sibling};

  pagx::FontConfig fontConfig;
  fontConfig.registerTypeface(typeface);
  doc->applyLayout(&fontConfig);

  // Middle Layer's height should reflect multi-line wrapping, not single-line.
  EXPECT_GT(middleLayer->layoutHeight, 30.0f);
  // Sibling should start after middle Layer + gap, not overlap.
  EXPECT_FLOAT_EQ(sibling->y, middleLayer->layoutHeight + 10.0f);
}

/**
 * Empty Layer with explicit height=100 in vertical layout. The explicit height should be preserved
 * even when stretch changes the cross-axis width, because height is not content-measured.
 */
PAGX_TEST(PAGXTest, EmptyLayerExplicitHeightInVerticalLayout) {
  auto doc = pagx::PAGXDocument::Make(400, 600);
  auto parent = doc->makeNode<pagx::Layer>();
  doc->layers.push_back(parent);
  parent->width = 400;
  ...
  EXPECT_FLOAT_EQ(child1->layoutHeight, 100.0f);
  EXPECT_FLOAT_EQ(child1->layoutWidth, 400.0f);
  EXPECT_FLOAT_EQ(child2->y, 110.0f);
}

/**
 * Flex child with stretch cross-axis in vertical layout. The flex main-axis is forced,
 * the cross-axis is forced by stretch. Neither should trigger content re-measurement.
 */
PAGX_TEST(PAGXTest, FlexChildNoContentRemeasure) {
  auto doc = pagx::PAGXDocument::Make(400, 600);
  auto parent = doc->makeNode<pagx::Layer>();
  doc->layers.push_back(parent);
  parent->width = 400;
  ...
  auto fixed = doc->makeNode<pagx::Layer>();
  fixed->height = 100;

  auto flexChild = doc->makeNode<pagx::Layer>();
  flexChild->flex = 1;

  parent->children = {fixed, flexChild};
  doc->applyLayout();

  EXPECT_FLOAT_EQ(fixed->layoutHeight, 100.0f);
  EXPECT_FLOAT_EQ(flexChild->layoutHeight, 500.0f);
  EXPECT_FLOAT_EQ(flexChild->layoutWidth, 400.0f);
  EXPECT_FLOAT_EQ(flexChild->y, 100.0f);
}

/**
 * Content-measured Layer with no children and no explicit size. setLayoutSize should not crash
 * or produce incorrect results when scanning empty contents.
 */
PAGX_TEST(PAGXTest, EmptyContentMeasuredLayerInLayout) {
  auto doc = pagx::PAGXDocument::Make(400, 600);
  auto parent = doc->makeNode<pagx::Layer>();
  doc->layers.push_back(parent);
  parent->width = 400;
  ...
  auto emptyLayer = doc->makeNode<pagx::Layer>();
  auto sibling = doc->makeNode<pagx::Layer>();
  sibling->height = 50;

  parent->children = {emptyLayer, sibling};
  doc->applyLayout();

  // Empty Layer should have 0 height, sibling starts at gap.
  EXPECT_FLOAT_EQ(emptyLayer->layoutHeight, 0.0f);
  EXPECT_FLOAT_EQ(sibling->y, 10.0f);
}
