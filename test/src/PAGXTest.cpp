...
/**
 * Test case: Unknown attributes on elements produce errors, while data-* and known attributes
 * are accepted without error.
 */
PAGX_TEST(PAGXTest, UnknownAttributeValidation) {
  // Unknown attribute on Layer element should produce an error.
  auto xml = R"(
    <pagx version="1.0" width="200" height="100">
      <Layer includeInLayout="false" bogusAttr="123">
  ...
  EXPECT_NE(doc->errors[0].find("Unknown attribute 'bogusAttr'"), std::string::npos);
  EXPECT_NE(doc->errors[0].find("'Layer'"), std::string::npos);

  // Unknown attribute on geometry element (Rectangle).
  xml = R"(
    <pagx version="1.0" width="200" height="100">
      <Layer>
        <Rectangle size="100,50" foo="bar"/>
      </Layer>
    </pagx>
  )";
  doc = pagx::PAGXImporter::FromXML(xml);
  ASSERT_TRUE(doc != nullptr);
  ASSERT_EQ(doc->errors.size(), 1u);
  EXPECT_NE(doc->errors[0].find("Unknown attribute 'foo'"), std::string::npos);
  EXPECT_NE(doc->errors[0].find("'Rectangle'"), std::string::npos);

  // Unknown attribute on color source element (SolidColor in Resources).
  xml = R"(
    <pagx version="1.0" width="200" height="100">
      <Resources>
        <SolidColor id="c1" color="#FF0000" intensity="5"/>
      </Resources>
    </pagx>
  )";
  doc = pagx::PAGXImporter::FromXML(xml);
  ASSERT_TRUE(doc != nullptr);
  ASSERT_EQ(doc->errors.size(), 1u);
  EXPECT_NE(doc->errors[0].find("Unknown attribute 'intensity'"), std::string::npos);

  // data-* attributes should NOT produce errors.
  xml = R"(
    <pagx version="1.0" width="200" height="100">
      <Layer data-role="hero" data-export="true">
        <Rectangle size="100,50" data-figma-id="abc"/>
        <Fill color="#FF0000" data-auto="true"/>
      </Layer>
    </pagx>
  )";
  doc = pagx::PAGXImporter::FromXML(xml);
  ...
  // All known attributes should NOT produce errors (comprehensive Layer check).
  xml = R"(
    <pagx version="1.0" width="400" height="300">
      <Layer name="test" visible="true" alpha="1" blendMode="normal"
             x="10" y="20" width="200" height="100"
             layout="horizontal" gap="5" flex="1" padding="10"
             alignment="center" arrangement="start" includeInLayout="true"
             left="0" right="0" top="0" bottom="0" centerX="100" centerY="50"
             antiAlias="true" groupOpacity="false" clipToBounds="false"
             passThroughBackground="true" preserve3D="false"
             maskType="alpha" rotation="45">
      </Layer>
    </pagx>
  )";
  doc = pagx::PAGXImporter::FromXML(xml);
  ...
  // Multiple unknown attributes on different elements accumulate errors.
  xml = R"(
    <pagx version="1.0" width="200" height="100">
      <Layer unknownA="1">
        <Ellipse size="50,50" unknownB="2"/>
        <Fill color="#000" unknownC="3"/>
      </Layer>
    </pagx>
  )";
  doc = pagx::PAGXImporter::FromXML(xml);
  ASSERT_TRUE(doc != nullptr);
  ASSERT_EQ(doc->errors.size(), 3u);
  std::string allErrors = doc->errors[0] + doc->errors[1] + doc->errors[2];
  EXPECT_NE(allErrors.find("unknownA"), std::string::npos);
  EXPECT_NE(allErrors.find("unknownB"), std::string::npos);
  EXPECT_NE(allErrors.find("unknownC"), std::string::npos);

  // Unknown attribute on root pagx element.
  xml = R"(
    <pagx version="1.0" width="200" height="100" theme="dark">
    </pagx>
  )";
  doc = pagx::PAGXImporter::FromXML(xml);
  ASSERT_TRUE(doc != nullptr);
