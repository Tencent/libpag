...
// ---------------------------------------------------------------------------
// SimplifyPaths (collinear-line collapsing)
// ---------------------------------------------------------------------------

CLI_TEST(PAGXOptimizerTest, SimplifyPathCollapsesColinearLines) {
  auto doc = PAGXDocument::Make(100, 100);
  auto* layer = AddTopLayer(doc.get());
  auto* group = doc->makeNode<Group>();
  auto* path = doc->makeNode<Path>();
  ...
  size_t before = data->verbs().size();
  PAGXOptimizer::Optimize(doc.get());

  // Path may have been canonicalized to a Rectangle (great); otherwise its verb count must shrink
  // since (10,0) and (20,0) are colinear with (0,0)/(30,0).
  bool simplified = false;
  for (auto* el : doc->layers[0]->contents) {
    Element* probe = el;
    if (el->nodeType() == NodeType::Group) {
      for (auto* inner : static_cast<Group*>(el)->elements) {
        if (inner->nodeType() == NodeType::Path || inner->nodeType() == NodeType::Rectangle) {
          probe = inner;
          break;
        }
      }
    }
    if (probe->nodeType() == NodeType::Rectangle) {
      simplified = true;
      break;
    }
    if (probe->nodeType() == NodeType::Path) {
      auto* outPath = static_cast<Path*>(probe);
      simplified = outPath->data->verbs().size() < before;
      break;
    }
  }
...
CLI_TEST(PAGXOptimizerTest, SimplifyPathDropsCollapsibleCubic) {
  auto doc = PAGXDocument::Make(100, 100);
  auto* layer = AddTopLayer(doc.get());
  auto* group = doc->makeNode<Group>();
  auto* path = doc->makeNode<Path>();
  auto* data = doc->makeNode<PathData>();
  data->moveTo(0, 0);
  // A cubic whose control points lie on the line between (0,0) and (40,0): degenerate to a Line.
  data->cubicTo(10, 0, 20, 0, 40, 0);
  data->lineTo(40, 40);
  data->close();
  path->data = data;
  group->elements.push_back(path);
  ...
  if (!doc->layers.empty() && !doc->layers[0]->contents.empty()) {
    Group* outGroup = nullptr;
    for (auto* el : doc->layers[0]->contents) {
      if (el->nodeType() == NodeType::Group) {
        outGroup = static_cast<Group*>(el);
        break;
      }
    }
    if (outGroup) {
      for (auto* el : outGroup->elements) {
        if (el->nodeType() == NodeType::Path) {
          for (auto verb : static_cast<Path*>(el)->data->verbs()) {
            if (verb == pagx::PathVerb::Cubic) {
              foundCubic = true;
            }
          }
        }
