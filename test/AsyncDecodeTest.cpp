 ...
 * 用例描述: 异步解码时候同步删除图层
 */
PAG_TEST_F(AsyncDecode, remove_ID79139135) {
  // 触发第一个视频图层初始化
  TestPAGPlayer->flush();
  // 移除所有图层
 ...
 */
PAG_TEST_F(AsyncDecode, init_ID79139125) {
  int currentFrame = 0;
 ...
 */
PAG_TEST_F(AsyncDecode, destory_ID79139125) {
  int currentFrame = 0;
  auto root = TestPAGPlayer->getComposition();
  while (currentFrame < root->stretchedFrameDuration()) {
    switch (currentFrame) {
 ...
 */
PAG_TEST_F(AsyncDecode, release_ID79427343) {
  TestPAGPlayer->flush();
...
PAG_TEST_F(AsyncDecode, bitmapCompDecodeTest) {
  //          25--------------------------60
  //    5----------------------45
  auto pagFile = LoadPAGFile("apitest/BitmapComp.pag");
 ...
 * 用例描述: 图片异步解码校验
 */
PAG_TEST_F(AsyncDecode, imageDecodeTest) {
  // 0----------15
  //         10-----------35
  //                           40---------60
  auto pagFile = LoadPAGFile("apitest/ImageDecodeTest.pag");
  EXPECT_TRUE(pagFile != nullptr);
  auto pagImage = MakePAGImage("apitest/imageReplacement.png");
